...
| P2 | **PITR / restore point noted.** | Owner records the current Neon PITR retention + earliest restore point (deploy-security §7). Migration is additive, but record it anyway. | PITR not enabled on the staging branch (note it; not a hard block for an additive change, but record). |
...
| any | Unexpected destructive surprise (not expected — the only DB change is additive) | **STOP**; restore via Neon PITR to the point noted in P2 | Neon PITR | No |

- **General:** because the only DB change is **additive**, a code rollback never needs a DB rollback (deploy-security §10). PITR is reserved for a genuine destructive surprise (none is expected here). **Auto-deploy stays disabled** through every rollback. **No rollback path requires touching any key variable.**
...
| **§8 rollback** | deploy-security §10 (additive ⇒ code rollback only); §7 (PITR) | safe rollback without DB/key change | — | Railway redeploy / Neon PITR | **Yes** |
