...
> **STATUS: DRAFT / PREPARATION ONLY — do NOT execute R1.** Awaiting Codex re-review + SHA-bound owner approval for every remaining step. **Executed so far: ONLY the pre-R1 recovery gates A1 + A2 + P1a (PASSED 2026-07-03 in a single owner-approved read-only session — §13.3.1) and D-R4a (the protected GitHub recovery branch, created + protected 2026-07-03 — §13.4; GitHub-only, no Railway change).** No other step has been run. R1 remains unmigrated, undeployed, and not staging-accepted; P1b/P8/P9 remain BLOCKED.
>
...
> **Status: amendment merged; A1 + A2 + P1a EXECUTED and PASSED 2026-07-03** (single owner-approved session — see the P1a execution record after §13.3). **Of §13.8 step 4, ONLY the GitHub portion (create + protect the recovery branch — D-R4a) is DONE (2026-07-03); the recovery deployment itself and everything after it (P8, P9, P1b, R1) are NOT started**; D-R4b (the Railway repoint, applied only by the recovery Deploy) and D-R5/D-E6 remain owner-gated. Codifies the owner-approved recovery architecture after the P1001 diagnosis. At diagnosis time (June 2026) staging was archived/suspended + over the then-Free monthly compute allowance; **Neon was upgraded to the LAUNCH plan (usage-based) on 2026-07-01 with an owner-set $20 spending limit (hard-stop enforcement UNVERIFIED; no further plan or limit change authorized)**; the Web service is Failed/non-serving (its pre-deploy `prisma migrate deploy` failed with **Prisma P1001** — a database **connection** failure; cause not isolated; pooled-host is a confirmed migration mismatch but is **not** proven to be the P1001 cause). Mirrors spec §20 + plan "Pre-R1 staging recovery". **This §13 splits the single P1 gate into P1a/P1b — that split is itself this reviewed amendment and must merge before execution.**
...
**Gate effects:** P1a PASSED; **D-R2 and D-R3 were executed for this session only** (any future resume or preflight needs fresh approval); **P1b, P8, P9 remain BLOCKED**; **D-R4 and D-R5/D-E6 remained OPEN and unstarted at the time of this record** (D-R4a — the GitHub branch + ruleset — was completed later the same day; §13.4. D-R4b/D-R5/D-E6 remain OPEN) — nothing downstream is promoted by this record.

### 13.4 Protected recovery branch
- **Status — D-R4a ✅ COMPLETE 2026-07-03 (GitHub only).** Branch **`recovery/pre-r1-baseline`** created directly at the verified SHA (remote tip verified `53bafac4716e8819b3a77ffb5a129bd6b25d59ef`; no new commit, no tree change). Dedicated repository ruleset **`protect-recovery-pre-r1-baseline` (id 18485943)**: enforcement **ACTIVE**; exact target `refs/heads/recovery/pre-r1-baseline` (no exclusions, no other branch affected); rules **deletion blocked + non-fast-forward/force-push blocked + ALL direct ref updates/commits blocked**; **zero configured bypass actors** (GitHub reports current-user bypass `never`); effective-rule evaluation on the branch returned exactly those three rules. The branch remains **not literally immutable** — a repository administrator can still edit or remove the ruleset itself — so the **exact-SHA verification below stays MANDATORY before and after every deployment or rebuild**. **Railway was NOT opened or changed: the source-branch repoint is D-R4b**, applied only by the separately approved recovery deployment's single Deploy click (apply-equals-deploy). **P8 is NOT established by this branch existing** — it still requires the successful baseline deployment + §13.5 health/smoke evidence + BOTH recovery-evidence legs.
...
| D-R4 | Create + protect the **recovery branch** at the verified SHA; approve **repointing Railway's connected branch** | **SPLIT: D-R4a (GitHub branch + ruleset) ✅ COMPLETE 2026-07-03** (`recovery/pre-r1-baseline` @ `53bafac4…`; ruleset `protect-recovery-pre-r1-baseline` id 18485943, active, zero bypass — §13.4). **D-R4b (the Railway repoint) OPEN, unexecuted, separately owner-gated** — it STAGES and is applied only by the recovery deployment's single Deploy click (apply-equals-deploy, §13.4) |
| D-R5 | Approve **disabling the auto pre-deploy migrate** for the recovery deployment | OPEN — note (2026-07-03): a standalone deploy-free disable does NOT exist on Railway (verified; a standalone staged attempt was safely DISCARDED the same day). The disable is staged WITH the recovery deployment and applied by its single Deploy; non-restoration afterwards follows the migration policy (§13.6 / rollout runbook §4.2, D-E6) |
