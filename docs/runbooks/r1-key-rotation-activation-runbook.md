...
- Placeholders used in this doc:
  - `<STAGING_DB_HOST>` — the staging Neon **direct** host (e.g. `ep-xxxx.eu-west-2.aws.neon.tech`), the value the §3.0 preflight must match. Used only to describe the confirmation; not typed into a secret.
...
|---|---|---|---|
| P1 | **Neon staging reachable on the verified DIRECT endpoint.** | Owner confirms the staging Neon branch (`br-ancient-water-…`, per staging runbook) is live; the injected `DATABASE_URL` is the **verified direct** (non-pooled) endpoint (D-3), confirmed by the §3.0 preflight. | Endpoint unreachable, or the host is a pooled/`-pooler` host (pooled endpoints are prohibited by this runbook — §2). |
...
- **Target confirmation is mandatory:** confirm the §3.0 preflight host = the **staging** host `<STAGING_DB_HOST>`, never production (deploy-security §7 — "a wrong URL is how accidents happen").
...
```
Expected: `host:` matches the known **staging** Neon **direct** host `<STAGING_DB_HOST>` and `db:` matches the staging database. **STOP** if either does not match staging, or if the host is a pooled/`-pooler` host (pooled endpoints are prohibited — §2).
...
| §3 migration | `migrate deploy` errors (permission / connectivity / pooled endpoint) | Confirm the §3.0 preflight host = the verified **direct staging** endpoint; re-run (`migrate deploy` is idempotent for already-applied entries) | None needed — no image deployed yet; table absent or not committed | No |
| §3 migration | Applied but `to_regclass` is null / table not visible | **STOP**; re-check you targeted the verified **direct staging** endpoint (§3.0), not a different branch | n/a | No |
...
|---|---|---|---|---|---|
| **P1–P9 preconditions** | staging runbook (Neon direct branch, 24 vars, worker offline, 🛑 do-not-recreate); deploy-security §7; `PROJECT-STATE.md` (Karaara UNVERIFIED / data to preserve); OD8 | all preconditions hold | any precondition fails (esp. P5 other-pending, P6 auto-deploy/worker, P8 no known-good Web target, P9 no test fixture) | n/a | **Yes** — owner confirms infra + records P8/P9 |
...
| **§2/§3.0 direct-endpoint** | `railway-backend-hosting-plan.md` D-3; migration.sql header; `prisma.config.ts` (`DATABASE_URL`) | §3.0 preflight host = verified **direct staging** host; pooled prohibited | host is pooled/`-pooler`; wrong (non-staging) host | n/a | **Yes** — owner injects/verifies the direct URL |
