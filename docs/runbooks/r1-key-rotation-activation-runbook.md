...
- Credential handling: the owner injected the runtime `DATABASE_URL` privately, clipboard-to-environment, **after two failed-safe attempts**; a zsh history scan returned **zero** hits; the clipboard was cleared and `DATABASE_URL` unset afterward. No credential entered chat, argv, or saved history at any point.
