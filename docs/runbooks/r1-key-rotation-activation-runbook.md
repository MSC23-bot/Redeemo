...
>
> **Source of truth for the code behaviour cited below:** `src/api/shared/keyring.ts`, `src/api/shared/keyringVerify.ts`, `src/worker.ts`, `src/index.ts`, `prisma/migrations/20260629000000_keyring_fingerprint/migration.sql`, `prisma.config.ts`, `package.json` (all on `main` @ the recorded current tip — **the `b66b0f95..current-main` window is NOT docs-only: it includes a substantive worker/maintenance rewrite (`src/worker.ts` + `src/api/queues/**`) and one additive migration (`20260702000000_maintenance_alert_types`), but NONE of it touches keyring/encryption (`src/api/shared/keyring.ts` / `encryption.ts` / `keyringVerify.ts` are unchanged since before `b66b0f95`), and ALL of it is already contained in the security checkpoint `cc507e6b` — so the R1 safety reasoning holds; `<R1_COMMIT>` must still be freshly re-anchored before R1 build/deployment** — see P3).
...
| P3 | **`main` is at the R1-capable tip; CI green.** | `git rev-parse origin/main` = the recorded current tip; CI green on it. **CORRECTED 2026-07-05 — the `b66b0f95..current-main` window is NOT docs-only.** `git diff --name-only b66b0f95 origin/main -- src/` = **17 files**, including a substantive worker/maintenance rewrite (`src/worker.ts` gains `createWorkerPrisma` / `buildMaintenanceRegistration` / the `--verify-keyring-and-exit` path; the whole `src/api/queues/maintenance*` set) **plus one additive migration** `20260702000000_maintenance_alert_types`. **CRUCIALLY: (a) NONE of it touches keyring/encryption** — `git diff b66b0f95 origin/main -- src/api/shared/keyring.ts src/api/shared/encryption.ts src/api/shared/keyringVerify.ts` is EMPTY; **and (b) ALL of it is already contained in the security checkpoint `cc507e6b`** (`b66b0f95` is an ancestor of `cc507e6b`; `git diff cc507e6b origin/main -- src/worker.ts src/api/queues/ prisma/schema.prisma prisma/migrations/` is EMPTY). So the R1 safety reasoning remains valid. **Re-anchor `<R1_COMMIT>` freshly before R1 build/deployment** (the CU-burn rollout runbook §4.3 / ledger D-M1/D-M2 owns the `<R1_COMMIT>` re-anchoring). | `src/` differs from `cc507e6b` in the keyring/encryption/worker/queues/schema R1 blast radius (checkpoint drift), OR keyring/encryption files differ from `b66b0f95`. |
