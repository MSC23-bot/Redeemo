...
| P8 | **(AMENDED — the rollback target is now a PROTECTED RECOVERY BRANCH at a verified pre-R1 SHA, with a defined evidence + retention model; see §13. The "available last-known-good deployment" framing below is superseded: no serving deployment currently exists, so P8 is established by deploying the protected pre-R1 branch and recording the §13 evidence.)** **A known-good Web rollback target is identified + recorded (owner verification, before activation).** Web is currently **Failed / non-serving**, so "redeploy the previous image" is NOT a safe assumption — the immediately-previous deployment may also be Failed/unavailable. | As a **separate owner verification step** (this runbook performs **no** Railway action now): (a) record the current Failed/non-serving Web baseline (deployment ID); (b) identify an **available last-known-good** Web deployment artifact + its exact commit / deployment ID (`<KNOWN_GOOD_WEB_TARGET>`); (c) confirm Railway can deliberately redeploy that artifact. Every §8/§12 Web rollback references **this recorded target**, not an unspecified "previous image". **Note:** a GitHub deployment-metadata query returning **no rows does NOT prove that no Railway deployment exists** (the environment name may differ, or Railway deploys may not register as GitHub deployments) — P8 still requires **later owner verification in Railway** (the authoritative deployment list). | **No** available known-good Web artifact can be identified — a rollback would restore only the current non-serving baseline. STOP for an owner decision **before** activation. |
...
| **P8 evidence + protected branch (§13.4-§13.5)** | this runbook §13; `git rev-parse 53bafac4` ✓ | protected branch at the verified SHA serves `/health` + read-only `GET /api/v1/customer/categories` smoke (status/shape/count only); deployment ID/SHA/timestamp; redeploy/rollback action + durable rebuild | branch tip ≠ verified SHA; baseline won't serve; no durable rebuild path | re-establish P8 if the retained image rollback disappears (retention-limited) | **Yes** — owner-approved provider branch-change + deploy |
...
- **Protection + SHA verification:** branch protection (no force-push, no direct commits, locked to the SHA); **verify `git rev-parse <recovery-branch>` == `53bafac4716e8819b3a77ffb5a129bd6b25d59ef` before and after the recovery deploy.**
...
4. a **visible Railway Redeploy/Rollback action** for the successful baseline deployment;
5. the **protected-branch source-rebuild fallback** (§13.4).
...
- **If the provider Redeploy/Rollback action disappears before R1 activation, P8 must be RE-ESTABLISHED** (re-deploy the protected branch and re-record the §13.5 evidence). The durable basis is the protected branch; the retained image action is a bonus only while present.

### 13.6 Long-term migration URL architecture (recommended; separate, owner-gated — see spec §20 / plan)
- **`MIGRATION_DATABASE_URL`** is the **recommended** CLI/migration URL; **runtime continues using the pooled `DATABASE_URL`** (`prisma.config.ts` reads `MIGRATION_DATABASE_URL ?? DATABASE_URL`; runtime `PrismaPg` keeps `DATABASE_URL`).
- **Fail-closed validation + fallback policy:** when `MIGRATION_DATABASE_URL` is set it MUST validate as a **direct (non-`-pooler`) URL** or boot/migrate **fails closed**; when unset, the migrate path **falls back to `DATABASE_URL`** with a clear warning (documented).
...
5. **P1b** (§13.2) — migration-readiness read-only preflight (direct). Gate for the migration.
6. **Restore Railway source branch to `main`/R1** (approved provider op); **apply the R1 migration on the DIRECT endpoint** (operator-run, §3) — only **after** P8 + P1b.
7. **R1 image deploy** (§4) → worker `--verify-keyring-and-exit` (§5) → fingerprint parity (§6) → acceptance (§7). **P9 fixture must exist before acceptance.**
8. **(Parallel / later, separate approval):** adopt `MIGRATION_DATABASE_URL` (§13.6); the permanent worker persistent-activity reduction → **WORKER RESTART** (not required for R1).

### 13.9 Decision ledger — unresolved owner decisions
| # | Decision | Status |
|---|---|---|
| D-R1 | Confirm **wait-for-Free-reset** (no paid upgrade) + headroom timing | OPEN |
...
| D-R6 | **P8 evidence model:** durable rebuild-from-branch only, or also a (retention-limited) retained image rollback | OPEN |
...
- **R1 migration not applied before P8 exists AND P1b passes.**
...
| Image rollback ≠ source rebuild | provider retention model + build-determinism limits | Web Failed; older deploys `REMOVED` | byte-identical not guaranteed; rollback ephemeral | P8 = protected branch + recorded serving deploy + durable rebuild; image rollback = bonus | Owner + Codex |
