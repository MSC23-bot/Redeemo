...
> **STATUS: DRAFT / PREPARATION ONLY — do NOT execute R1.** Awaiting Codex re-review + SHA-bound owner approval for every remaining step. **Executed so far (all 2026-07-03, each owner-approved): A1 + A2 + P1a (PASSED — §13.3.1); D-R4 (the protected GitHub recovery branch — §13.4); and the recovery deployment itself (D-R4b + D-R5/D-E6 via one owner-clicked Deploy, ~21:02 UTC) — the pre-R1 baseline is SERVING and P8 is ESTABLISHED (§13.5.1).** No R1 step has been run. R1 remains unmigrated, undeployed, and not staging-accepted; **P1b and P9 remain BLOCKED; D-R10 remains OPEN**.
>
...
>
> **Source of truth for the code behaviour cited below:** `src/api/shared/keyring.ts`, `src/api/shared/keyringVerify.ts`, `src/worker.ts`, `src/index.ts`, `prisma/migrations/20260629000000_keyring_fingerprint/migration.sql`, `prisma.config.ts`, `package.json` (all on `main` @ the recorded current tip — **historical audit pinned to the 2026-07-05 tip `aaf7f382`: the `b66b0f95..aaf7f382` window was NOT docs-only (a worker/maintenance rewrite in `src/worker.ts` + `src/api/queues/**` and one additive migration `20260702000000_maintenance_alert_types`), but NONE of it touched keyring/encryption (`src/api/shared/keyring.ts` / `encryption.ts` / `keyringVerify.ts` unchanged since before `b66b0f95`), and ALL of it was already contained in the security checkpoint `cc507e6b` — so the R1 safety reasoning held at that tip. This is a point-in-time fact, not a timeless claim; the binding check is the EXECUTION-TIME P3 GATE (fresh `cc507e6b..<R1_COMMIT>` + `b66b0f95..<R1_COMMIT>` comparisons, Fable-adjudicated) before R1 build/deployment** — see P3).
...
- **For migration / probe commands, `DATABASE_URL` (the staging Neon direct URL — in the CONTROLLED OPERATOR PROCESS ONLY; NOT Railway's pooled runtime `DATABASE_URL`) is injected into the operator session environment by the secret store BEFORE any command is run.** It is never typed into a command line, never shown inline (no `DATABASE_URL=… <cmd>` form), and never displayed. Confirm the target with the **credential-safe preflight** (§3.0), which prints **only** the host + database identifier — never username, password, query parameters, or the full URL.
...
|---|---|---|---|
| P1 | **Staging target identity — SPLIT into `P1a` (runtime-recovery) + `P1b` (migration-readiness); see §13.** The Neon **control-plane identifiers have been observed and recorded** (project / branch / endpoint / database / migration-role — owner-held, **not committed here**). The **runtime binding + reachability** (`P1a`) was **confirmed 2026-07-03 — PASS (§13.3.1)**; the **migration binding + role/permissions** (`P1b`) remains **gated**. | `P1a` (pooled runtime) is confirmed **before serving** the pre-R1 baseline (§13.1); `P1b` (direct, non-`-pooler` migration endpoint + role/grants) is confirmed **before any migration** (§13.2) — each via a separately-approved **minimum read-only preflight** (§13.3); no secret is printed. | **STOP** if the recorded identity cannot be re-verified from the control plane; `P1a`/`P1b` cannot be confirmed on their **required** endpoints (`P1a` **pooled**, `P1b` **direct**); or any **production identifier** appears. |
...
| P3 | **`main` is at the R1-capable tip; CI green.** | `git rev-parse origin/main` = the recorded current tip; CI green on it. **HISTORICAL AUDIT — pinned to the 2026-07-05 audited tip `aaf7f3824f7704ebeddc38da65cb5d2ca012a346` (NOT the moving `origin/main`, which continuous Merchant work advances):** the `b66b0f95..aaf7f382` window is **NOT docs-only** — `git diff --name-only b66b0f95 aaf7f382 -- src/` was **17 files** at that tip, including a substantive worker/maintenance rewrite (`src/worker.ts` gains `createWorkerPrisma` / `buildMaintenanceRegistration` / the `--verify-keyring-and-exit` path; the whole `src/api/queues/maintenance*` set) **plus one additive migration** `20260702000000_maintenance_alert_types`. **At `aaf7f382`: (a) NONE of it touched keyring/encryption** — `git diff b66b0f95 aaf7f382 -- src/api/shared/keyring.ts src/api/shared/encryption.ts src/api/shared/keyringVerify.ts` was EMPTY; **and (b) ALL of it was already contained in the security checkpoint `cc507e6b`** (`b66b0f95` is an ancestor of `cc507e6b`; `git diff cc507e6b aaf7f382 -- src/worker.ts src/api/queues/ prisma/schema.prisma prisma/migrations/` was EMPTY), so the R1 safety reasoning held at that tip. *(The "17 files" count is a point-in-time fact at `aaf7f382`, not a timeless claim — later Merchant work will change it.)* | (historical) `src/` at `aaf7f382` differed from `cc507e6b` in the keyring/encryption/worker/queues/schema R1 blast radius, OR keyring/encryption differed from `b66b0f95`. **See the separate EXECUTION-TIME P3 GATE below — that is the gate that actually blocks R1.** |
| P3 (execution-time gate) | **The R1 build/deploy source is verified against the checkpoint at the moment R1 runs — NOT against a historical audit.** | At R1 execution: (1) resolve the eventual **`<R1_COMMIT>`** (re-anchored per the CU-burn rollout runbook §4.3 / ledger D-M1/D-M2); (2) **freshly compare `cc507e6b..<R1_COMMIT>` across worker + queues + schema + migrations** (`git diff --stat cc507e6b <R1_COMMIT> -- src/worker.ts src/api/queues/ prisma/schema.prisma prisma/migrations/`); (3) **freshly compare `b66b0f95..<R1_COMMIT>` across `keyring.ts` + `encryption.ts` + `keyringVerify.ts`**; (4) **Fable adjudicates every new difference** either check surfaces. | **STOP if any unexplained R1-blast-radius change appears** in the fresh `cc507e6b..<R1_COMMIT>` comparison (worker/queues/schema/migrations), OR any change at all appears in the `b66b0f95..<R1_COMMIT>` keyring/encryption comparison — until Fable has adjudicated it. |
...
| P8 | **Web rollback baseline = a PROTECTED RECOVERY BRANCH at the verified pre-R1 SHA `53bafac4…`; see §13.4–§13.5.** **P8 ESTABLISHED 2026-07-03** — the recovery deployment `6d26b0b4…` (built from the protected branch) is SERVING; full evidence at §13.5.1. (Historical: before it, no serving deployment existed — history was all `FAILED`/`REMOVED`.) | Deploy the protected branch (GitHub ruleset; SHA verified before every deploy/rebuild); record **BOTH** (i) a **currently-visible Railway Redeploy/Rollback action** AND (ii) the **durable protected-branch source-rebuild path** — plus `/health` liveness + a read-only `GET /api/v1/customer/categories` smoke (status/shape/count only) + deployment ID/SHA/timestamp (§13.5). | **STOP** if the baseline will not serve; the branch tip ≠ the verified SHA; **either** required evidence leg is missing; or the provider Redeploy/Rollback action later disappears — P8 then becomes **BLOCKED** and must be re-established. |
...
- **Direct endpoint is mandatory (operational rule):** this runbook's Prisma migration procedure **requires and supports only the verified Neon direct endpoint**; **pooled endpoints are prohibited by this runbook**. (`railway-backend-hosting-plan.md` D-3 + the migration header pin the direct endpoint for migrations.) **Railway's runtime `DATABASE_URL` is the POOLED endpoint and MUST stay pooled** — the app serves on it, and `P1a` verifies that pooled runtime binding. `prisma.config.ts` reads `DATABASE_URL` from **whatever process runs `prisma migrate`**, and today there is **no separate `directUrl` / `MIGRATION_DATABASE_URL`** — so the bounded R1 migration is run by the **operator from a controlled host whose OWN process `DATABASE_URL` is a SEPARATELY-INJECTED DIRECT endpoint credential** (distinct from — and never a change to — Railway's pooled runtime value), verified via `P1b` + the §3.0 preflight. (The long-term `MIGRATION_DATABASE_URL` automation in §13.6 is **not** authorized yet.) Confirm the endpoint via the §3.0 preflight before running any migration command.
...
> **Secret handling:** for these migration commands, `DATABASE_URL` is the staging Neon **direct** URL **— but only within this controlled operator process environment.** It is **NOT** Railway's runtime `DATABASE_URL`, which is **pooled** and stays pooled/unchanged. The operator-process `DATABASE_URL` (= direct) is **injected by the secret store before any command is run** — never typed into a command line, shell history, chat, or this doc. Run the commands below **bare** (they read the injected `DATABASE_URL` from the operator process env). Do **not** prefix them with `DATABASE_URL=…`, and **do not** change Railway's runtime `DATABASE_URL`.
...
- **Merged-worker precondition (Neon CU-burn worker-pool mechanism, on `main` since `1f07d901…`):** the worker now **fail-closes without `WORKER_DATABASE_POOL_MAX`** for EVERY start mode — **including `--verify-keyring-and-exit`** (it is resolved before the probe's early exit; `src/worker.ts` / `src/api/shared/env.ts`). The operator env for this probe MUST therefore also carry a valid `WORKER_DATABASE_POOL_MAX` (integer 1-10; value owner-gated — see `docs/runbooks/neon-cu-burn-maintenance-rollout-runbook.md` §3.3). The `MAINTENANCE_*` scheduler values are NOT required for the probe (it exits before maintenance config is resolved).
...
| §5 Worker probe | `--verify-keyring-and-exit` exits **1** (publish failed) | Check DB reachability + that the operator process has the worker's `ENCRYPTION_KEY` + the injected **direct** `DATABASE_URL` (operator process, **NOT** Railway runtime pooled); fix and **re-run the probe**. Do **not** infer parity from a non-zero exit | re-run probe (no daemon started) | No |
...
**Historical mechanism (the OLD image):** the pre-fix worker daemon ran repeatable BullMQ sweeps — email delivery, outbox-reconciler (60s), claim-stale (hourly), promote-pending-hours (60s) (constants `RECONCILE_EVERY_MS=60000`, `PROMOTE_PENDING_HOURS_EVERY_MS=60000`, claim-stale hourly). **Source proved persistent 60-second / hourly worker database activity; these repeatable sweeps are the LEADING IDENTIFIED persistent-activity mechanism of the Neon compute usage — NOT a telemetry-proven sole cause** (Web serving and other activity also consume compute).

**Remediation status (2026-07-02):** the permanent persistent-activity reduction is **CODE-COMPLETE on `main`** (`1f07d901…` — Neon CU-burn PR-A..PR-D + the CI-truthfulness fix + the explicit `WORKER_DATABASE_POOL_MAX` pool mechanism): the three repeatables are deleted from the code and every recurring sweep runs on the fail-closed process-local maintenance scheduler. **The hold below still stands** — code-merge alone satisfies nothing operationally. The worker restart is gated by the separately-approved staging activation + 48-72 h acceptance in **`docs/runbooks/neon-cu-burn-maintenance-rollout-runbook.md`** (which also reconciles this runbook's P3 `<R1_COMMIT>` re-anchoring and P5 pending-set expansion — its §4.3 / ledger D-M1/D-M2). **Permanent worker remediation gates the worker RESTART only — it does NOT gate the bounded Web/R1 recovery, which proceeds with the worker Offline (§13).** Until that staging activation is completed AND owner-accepted, the worker MUST remain **Offline**:
...
| **P8 Web rollback target** | staging runbook; deploy-security §10; **§13.5.1 (2026-07-03: known-good artifact EXISTS — deployment `6d26b0b4…` Active with a visible Redeploy action)** | a known-good Web artifact + commit/deployment ID is recorded; Railway can redeploy it — **satisfied 2026-07-03; RE-VERIFY the visible action before activation (retention rule §13.5)** | the visible Redeploy/Rollback action has disappeared (P8 ⇒ BLOCKED, re-establish) | n/a (verification step; no Railway action now) | **Yes** — owner re-verification in Railway before activation |
...
| **P8 evidence + protected branch (§13.4-§13.5)** | this runbook §13; `git rev-parse 53bafac4` ✓ | protected branch (GitHub ruleset; SHA verified before EVERY deploy/rebuild) serves `/health` + read-only `GET /api/v1/customer/categories` smoke (status/shape/count only); deployment ID/SHA/timestamp; **BOTH** a currently-visible Railway redeploy/rollback action AND the durable protected-branch rebuild (D-R6) | branch tip ≠ verified SHA; baseline won't serve; no durable rebuild path; **the visible redeploy/rollback action is absent** | **P8 BLOCKED + re-established** if the provider redeploy/rollback action disappears before R1 | **Yes** — the owner-approved branch change is STAGED and applied by the single deliberate Deploy (apply-equals-deploy, §13.4; staged set verified first) |
...
> **Status: amendment merged; A1 + A2 + P1a EXECUTED and PASSED 2026-07-03** (single owner-approved session — see the P1a execution record after §13.3). **§13.8 step 4 is FULLY EXECUTED (2026-07-03): D-R4 complete (branch + protection + the Railway repoint), D-R5/D-E6 executed (pre-deploy hook removed), the recovery deployment succeeded, the baseline is SERVING, and P8 is ESTABLISHED (§13.5.1). Steps 5 onward (P9 fixture, P1b, R1 migration/deployment) are NOT started**; D-R10 remains OPEN. Codifies the owner-approved recovery architecture after the P1001 diagnosis. At diagnosis time (June 2026) staging was archived/suspended + over the then-Free monthly compute allowance; **Neon was upgraded to the LAUNCH plan (usage-based) on 2026-07-01 with an owner-set $20 spending limit (hard-stop enforcement UNVERIFIED; no further plan or limit change authorized)**; the Web service was Failed/non-serving until the 2026-07-03 recovery deployment (its pre-deploy `prisma migrate deploy` had failed with **Prisma P1001** — a database **connection** failure; cause not isolated; pooled-host is a confirmed migration mismatch but is **not** proven to be the P1001 cause). Mirrors spec §20 + plan "Pre-R1 staging recovery". **This §13 splits the single P1 gate into P1a/P1b — that split is itself this reviewed amendment and must merge before execution.**

### 13.1 P1a — runtime-recovery gate (serve the pre-R1 baseline; NO migration)
**Status: ✅ PASSED 2026-07-03** (attempt 1 of 3; full evidence in the P1a execution record after §13.3). A fresh P1a re-verification is still required if provider state materially changes before the recovery deployment.
...
### 13.3.1 P1a execution record — 2026-07-03 (owner-approved session; D-R2 + D-R3 consumed for this session)

**Verdict: P1a PASS** (attempt 1 of 3; well inside the 15-minute window; no retry needed).

| Phase | Evidence (sanitized) |
|---|---|
| A1 headroom | Neon LAUNCH, usage-based; charges **$0.00 before AND after**; "**$0.00 of $20 spending limit reached**" before and after; no billing/limit warning |
| Step A (no connection) | endpoint **`ep-round-wave-abpnesg3`** (exact match); database **`neondb`** (owner-confirmed vs the private P1 mapping); **class POOLED**; no production identifier |
| Step B (§13.3 heredoc, attempt 1) | `reachable: YES` · `current_database: neondb` · **exit=0** · **no cleanup fallback line** (graceful rollback + disconnect) · no raw error or credential in output |
| Phase 5 (~14 min later) | staging compute back to **Idle** (scale-to-zero worked); project CU counter unchanged at 110.59; billing unchanged at $0.00; **no visible usage/spend delta** |

**Audit record (honest, exact):**
- Credential handling (exact chronology): (1) the runtime `DATABASE_URL` was initially pasted directly at the interactive zsh prompt; zsh returned `no matches found` and **aborted before any child-process execution or database connection** — the value reached local interactive shell input/scrollback ONLY, never a child-process argv and never the network; (2) the URL was removed before anything was submitted to chat; (3) the `~/.zsh_history` safety scan returned **zero** matches; (4) terminal scrollback and the clipboard were cleared; (5) two subsequent clipboard-to-environment validation attempts **failed closed** because the clipboard did not contain a valid PostgreSQL URL; (6) the final clipboard-to-environment attempt succeeded — the value remained **environment-only**, never entering argv or printed output; (7) the clipboard was immediately cleared; (8) `DATABASE_URL` was later unset. Net containment: no credential was submitted to chat or found in saved zsh history; the only exposure was local interactive shell input/scrollback, which was subsequently cleared.
- No database/schema/application mutation occurred — two read-only SELECTs inside a rolled-back read-only transaction.
- The approved connection **automatically unarchived the `staging` branch** (expected). The **`production` parent branch row also flipped to unarchived** — a control-plane archive-flag side effect; production's **compute did not wake** and its last-active time was unchanged. **Do NOT manually re-archive either branch** — Neon re-archives on its own inactivity schedule; confirm on a later check.
- **Interpretation limit:** current pooled reachability does **NOT establish the historical P1001 cause** — it only proves the pooled endpoint serves a read-only connection today; the original failure's cause remains un-isolated.
- **Open anomaly (follow-up trigger):** the project-level "**110.59 CU-hrs** since Jul 1" counter remains inconsistent with every per-branch counter (0), the org-level summary (0), and $0.00 billing. Follow up within the CU-burn investigation the next time the dashboard is legitimately open, and BEFORE relying on that counter for any future A1/usage decision.
- **SSL follow-up:** the `pg` driver's pre-v9 SSL-mode aliasing (its deprecation warning fired during Step B) is a future follow-up when the driver is next upgraded — no action now.

**Gate effects:** P1a PASSED; **D-R2 and D-R3 were executed for this session only** (any future resume or preflight needs fresh approval); **P1b, P8, P9 remained BLOCKED at the time of this record**; **D-R4 and D-R5/D-E6 remained OPEN and unstarted at the time of this record** (all subsequently executed later the same day: D-R4 §13.4; D-R5/D-E6 + the recovery deployment + P8 §13.5.1. **P1b and P9 still remain BLOCKED**) — nothing downstream was promoted by this record.

### 13.4 Protected recovery branch
- **Status — D-R4 ✅ COMPLETE 2026-07-03: D-R4a (GitHub branch + protection) AND D-R4b (the Railway repoint, applied by the owner-clicked recovery Deploy — §13.5.1).** Branch **`recovery/pre-r1-baseline`** created directly at the verified SHA (remote tip verified `53bafac4716e8819b3a77ffb5a129bd6b25d59ef`; no new commit, no tree change). Dedicated repository ruleset **`protect-recovery-pre-r1-baseline` (id 18485943)**: enforcement **ACTIVE**; exact target `refs/heads/recovery/pre-r1-baseline` (no exclusions, no other branch affected); rules **deletion blocked + non-fast-forward/force-push blocked + ALL direct ref updates/commits blocked**; **zero configured bypass actors** (GitHub reports current-user bypass `never`); effective-rule evaluation on the branch returned exactly those three rules. The branch remains **not literally immutable** — a repository administrator can still edit or remove the ruleset itself — so the **exact-SHA verification below stays MANDATORY before and after every deployment or rebuild**. **D-R4b was subsequently EXECUTED the same day**: the repoint was staged with the pre-deploy-command removal and applied by the single owner-clicked recovery Deploy (apply-equals-deploy; §13.5.1). Railway's Web source is now the recovery branch (deliberately NOT restored to `main` — the restore belongs to the future R1 deployment, §13.8 step 8). **P8 was established by the successful deployment + §13.5 evidence, not by the branch existing.**
...
- **Mechanism:** Railway GitHub services deploy from a **configured branch, not a tag.** Create a **dedicated, protected GitHub branch fixed at that SHA** — ✅ created 2026-07-03: `recovery/pre-r1-baseline`.
- **Protection + SHA verification:** the recovery branch is **NOT "literally immutable"** (GitHub provides no true immutability) — enforce a **GitHub repository ruleset** on it: **no force-push, no direct updates/commits**, restricted updaters — ✅ implemented 2026-07-03 as ruleset id 18485943 (deletion + non-fast-forward + update all blocked; zero bypass actors). **Verify `git rev-parse <recovery-branch>` == `53bafac4716e8819b3a77ffb5a129bd6b25d59ef` BEFORE every deployment OR rebuild** (and re-verify after each); a tip that differs from the SHA is a STOP.
- **Railway source-branch change + restoration:** **pointing Railway's Web service at the recovery branch is a separately-approved provider change**; after recovery, **restoring the Web service's source branch to `main` (R1 code) is a second separately-approved provider change.** **Auto-deploy stays DISABLED** throughout. **Apply-equals-deploy (verified live 2026-07-03):** each of these edits only STAGES on Railway; it takes effect exclusively when the operator clicks **Deploy**, which commits the staged configuration AND starts a deployment in one action. A staged-but-unapplied branch change is a hazard (it rides along with the next Deploy anyone clicks), so each branch change is staged immediately before — and applied by — its own deliberately approved deployment, never parked.
- **Railway staged-change safety (corrected 2026-07-03 — verified live):** Railway provides **NO deploy-free save/apply** for Web service configuration. Editing the **source branch** or the **pre-deploy command** creates a STAGED change ("Apply N changes"); the ONLY apply affordance is a button labelled **Deploy** (commits the config AND starts a deployment), and the staged-changes menu offers only **Discard Changes**. The safety discipline is therefore: (1) stage together EXACTLY the owner-approved changes for the upcoming deliberate deployment; (2) verify the staged set contains only those changes and no Variables or unrelated setting — anything unexpected ⇒ **Discard + STOP**; (3) click Deploy ONCE as the deliberately approved deployment — the same click applies the staged configuration and starts the intended deployment; (4) any deployment that appears WITHOUT that deliberate Deploy click is an incident ⇒ stop and report. (A standalone deploy-free pre-deploy-command disable was attempted and safely DISCARDED on 2026-07-03 — no provider state changed.)
- **No R1 migration during baseline recovery:** the migration is absent from the source **and** the pre-deploy migrate command's removal is **staged together with the recovery deployment and applied by its single Deploy click** (apply-equals-deploy — there is no separate earlier disable step). Whether the command is later restored remains governed by the migration policy (§13.6 / the CU-burn rollout runbook §4.2: NOT restored; permanent automation stays gated on `MIGRATION_DATABASE_URL`, D-R8).

### 13.5 P8 evidence + retention model (require ALL of)
**Status: ✅ P8 ESTABLISHED 2026-07-03** — every item below passed; full record at §13.5.1. The retention rule stays live: if the visible provider Redeploy/Rollback action disappears before R1 activation, P8 becomes BLOCKED and must be re-established.
...
### 13.5.1 Recovery-deployment + P8 execution record — 2026-07-03 (owner-approved session)

**One owner-clicked Deploy applied exactly two staged changes** (verified in Railway's "2 changes to apply" dialog before the click): Web source branch `main` → `recovery/pre-r1-baseline`; Web pre-deploy command `npx prisma migrate deploy` → removed/empty. No other staged change existed; no pre-deploy migration ran (stage progression: build → healthchecks → success; the old failure stage is gone).

| Item | Evidence |
|---|---|
| Deployment | Railway staging, ID `6d26b0b4-38ce-4f03-bba7-acaf620a2cd8`, started ~2026-07-03 21:02 UTC, **"Deployment successful" → Active/Online** — the first successful Web deployment (prior history all FAILED/REMOVED) |
| Built-SHA provenance (honest) | Railway displayed the **matching #339 squash-commit message, not the full SHA**. Exact identity `53bafac4716e8819b3a77ffb5a129bd6b25d59ef` is supported by the chain: branch verified at the exact SHA BEFORE the deploy → the staged source-branch change selected that branch → Railway showed the matching commit message → the branch re-verified at the same exact SHA AFTER the deploy → ruleset 18485943 prevented any movement in between |
| Liveness | `GET /health` → HTTP 200 `{"status":"ok"}` |
| Bounded DB-backed smoke | read-only `GET /api/v1/customer/categories` → HTTP 200; object containing a `categories` list; **14 items**; expected item shape; **no response content or owner data recorded** |
| P8 leg (i) durable rebuild | `recovery/pre-r1-baseline` remained at the exact SHA; ruleset `protect-recovery-pre-r1-baseline` (id 18485943) ACTIVE and unchanged — re-verified after the deployment |
| P8 leg (ii) provider action | the successful deployment's actions menu visibly offered **Redeploy** (View logs / Restart / Redeploy / Remove) — read and closed, nothing clicked |
| Post-deploy cost/state (~15 min) | staging compute **returned to Idle** (the serving app's pooled connections do not hold it awake; the first request after scale-to-zero pays a cold-start); usage 0.02 CU-hrs since Jul 1; billing **$0.01 of $20**, no warning; `staging` + `production` branch rows remain unarchived; production compute Idle (last active ~6 days). The earlier 110.59 CU-hrs display is confirmed stale/anomalous by the refreshed counters |
| Boundaries held | auto-deploy stayed disabled; worker stayed Offline; no logs/Variables/secrets/connection strings opened; no migration or schema/data mutation; PR #338 untouched |

**Execution audit:** Claude auto-mode refused the Deploy action twice (subagent, then lead agent) — the permission system requires the human on the irreversible click; **the owner personally clicked Deploy exactly once**; no bypass/API/CLI workaround was used. One intercepted UI click was recovered by navigation with no mis-click. Temporary snapshot artifacts created contrary to task boundaries were moved out of the repository; the tree was verified clean.

**Operational follow-up (recorded, not a failure):** the Railway workspace trial banner read approximately **"10 days or $2.99 left"** — the hosting-plan decision bounds how long this baseline stays hosted; it also makes the §13.5 retention rule (visible Redeploy action can disappear) time-relevant.

**Gate effects:** D-R4 COMPLETE (a + b) · D-R5/D-E6 EXECUTED (the hook is absent and is NOT restored under current policy — §13.6/rollout §4.2) · recovery baseline SERVING · **P8 ESTABLISHED** · **P9 BLOCKED/unstarted · D-R10 OPEN · P1b BLOCKED/unstarted · R1 migration/deployment unstarted** — P8 completion approves NONE of them.
...
7. **Apply the R1 migration on the DIRECT endpoint FIRST** (operator-run, §3) — only **after** P8 + **P9** + P1b. Do **NOT** stage or apply the Railway source-branch restoration before the migrations are applied: restoring the branch is a staged change whose only apply action is Deploy, so applying it early would immediately deploy the R1 image against an unmigrated schema (apply-equals-deploy, §13.4).
8. **THEN stage the Web source-branch restoration to `main`/R1** (keep the pre-deploy migrate command absent — it is not restored), verify the staged set (§13.4), and **click Deploy ONCE as the deliberately approved R1 image deployment** (§4) → worker `--verify-keyring-and-exit` (§5) → fingerprint parity (§6) → acceptance (§7) — the P9 fixture (step 5) is already in place.
...
| D-R2 | Approve the **control-plane unarchive/resume** (A2) | **EXECUTED 2026-07-03** for the single approved P1a session (the Step B probe was the controlled resume; §13.3.1). Any FUTURE resume needs fresh owner approval |
| D-R3 | Approve the **separately-scoped minimum read-only preflight** (P1a now; P1b later) | **EXECUTED 2026-07-03 for P1a** (PASS; §13.3.1). The **P1b preflight portion remains OPEN/BLOCKED** and needs its own approval |
| D-R4 | Create + protect the **recovery branch** at the verified SHA; approve **repointing Railway's connected branch** | **✅ COMPLETE 2026-07-03 (both halves): D-R4a** (`recovery/pre-r1-baseline` @ `53bafac4…`; ruleset id 18485943 — §13.4) **+ D-R4b EXECUTED** (repoint staged with the hook removal, applied by the single owner-clicked recovery Deploy — §13.5.1) |
| D-R5 | Approve **disabling the auto pre-deploy migrate** for the recovery deployment | **✅ EXECUTED 2026-07-03** — the removal was staged WITH the recovery deployment and applied by its single owner-clicked Deploy (§13.5.1). The hook is **absent and NOT restored** under current policy (§13.6 / rollout runbook §4.2, D-E6). (Historical note: a standalone deploy-free disable does not exist on Railway; the earlier standalone attempt was DISCARDED) |
...
|---|---|---|---|---|---|
| Railway deploys from a branch, not a tag | provider model | **`recovery/pre-r1-baseline` created + protected 2026-07-03 (D-R4a; §13.4)** | unsupported tag mechanism | **protected recovery branch** at the verified SHA — ✅ done; the Railway repoint is D-R4b at the recovery Deploy | Owner + Codex |
| Recovery SHA | `git rev-parse 53bafac4` = `…716e8819…` ✓ | **remote `recovery/pre-r1-baseline` tip verified = the exact SHA (2026-07-03)** | durable pin in place (ruleset 18485943) — still not literally immutable, so pre/post-deploy SHA verification stays mandatory | branch protection at the verified SHA — ✅ done (D-R4a) | Owner |
| Image rollback ≠ source rebuild | provider retention model + build-determinism limits | **2026-07-03: deployment `6d26b0b4…` Active with a visible Redeploy action; P8 ESTABLISHED (§13.5.1)** (older deploys `REMOVED`) | byte-identical not guaranteed; rollback ephemeral (trial-plan clock makes retention time-relevant) | P8 requires **BOTH** the durable protected-branch rebuild **and** a currently-visible Railway redeploy/rollback action (D-R6); BLOCKED + re-established if the provider action disappears | Owner + Codex |
...
| Headroom / archived | P1 dashboard | **2026-07-03: staging woke for P1a and returned to Idle; `staging` + `production` branch rows now UNARCHIVED (flag side effect — production's compute never woke); do not manually re-archive** | recovery needs cost headroom — **A1 checked 2026-07-03: $0.00 of $20, no warnings** | **LAUNCH (usage-based) since 2026-07-01 + $20 limit (enforcement UNVERIFIED); fresh A1 re-check before the recovery deployment; no further plan/limit change** | Owner |
| P1001 cause | owner log (sanitised) | connection fails at pre-deploy; **2026-07-03: the pooled endpoint served a read-only connection (P1a PASS) — current reachability does NOT establish the historical cause** | cause not isolated | fix reachability + direct path, re-attempt, re-diagnose if persists | Owner + Codex |
