...
- **Migrations:** run **`npx prisma migrate deploy`** against the environment's DB **before** the new code serves traffic (it applies committed migrations, never generates new ones). The **`Procfile` intentionally has NO `release:` line** — migrations are kept as an explicit, documented pre-deploy step for portability + control (not every platform runs a Heroku-style release phase, and an auto-applied migration is harder to gate / roll back). On a NON-Neon-backed platform, a pre-deploy/release phase **may** run `npx prisma migrate deploy` there instead; otherwise run it as a manual pre-deploy step (see §4). **Neon-backed staging/production caveat (applies to THIS project's staging + production):** a provider pre-deploy phase inherits the service's runtime `DATABASE_URL`, which on Railway is the **pooled** endpoint — and pooled migrations are prohibited (they are the recorded P1001 failure path; r1 runbook §2/§13). Until the separately-gated `MIGRATION_DATABASE_URL` workstream (r1 §13.6 / D-R8) is implemented and verified fail-closed, keep the provider pre-deploy migrate **disabled** on Neon-backed environments and run migrations from the controlled operator process on the verified **direct** endpoint (see the CU-burn rollout runbook §4.2). The two Phase-0 migrations (`CommunicationStatus += QUEUED` + `BranchPhoto` moderation fields) are **additive** — a code rollback leaves harmless extra columns/values, no data loss (§10).
- **Node:** **split pin (do not conflate).** Repo root / backend / Railway / CI = **Node 24** (Active LTS) via the root `.nvmrc` — the host honours it (Nixpacks/Railway read `.nvmrc`). `apps/customer-app/.nvmrc` = **Node 20.19.4** for the Expo / jest-expo toolchain until separately verified (jest-expo had reliability issues on Node 24); that pin is a local build/test toolchain, not a hosted runtime, so its EOL-Node-20 bump is a deferred follow-up — never block backend / Railway work on it. (Node 20 reached production end-of-life 30 Apr 2026, which is why the hosted backend moved to 24 — PR #227.)
...
**Resend / email (pipeline WIRED; delivery still owner-gated DARK via `EMAIL_ENABLED`; sender policy DECIDED, D-F 2026-06-09):** the outbox pipeline is now wired (this is NO longer `console.info` placeholders): `notify()` (`src/api/shared/notify.ts`) commits a QUEUED `CommunicationLog` outbox row, the delivery worker (`src/api/queues/processors/email.ts`) plus the outbox reconciler send through the Resend wrapper (`src/api/shared/email.ts`), and the bounce/complaint webhook (`src/api/webhooks/resend.ts`) resolves rows by `externalId`. The whole path stays DARK while `EMAIL_ENABLED` is off: the wrapper returns `{ skipped: true, reason: 'disabled' }` (no client, no `RESEND_API_KEY` needed) and the worker records each row terminal `FAILED` (NOT a held backlog, so flipping on later never flushes stale reset links). The §SEC.1 send-abuse controls live in code: the atomic limiter tiers + global daily cost gate (`src/api/shared/emailLimiter.ts`, GAP-1..GAP-4), the email-OTP resend cooldown + failed-round escalation (GAP-5), and the auto send-pause + send-volume counters + SUPER_ADMIN ops view (`src/api/shared/emailOps.ts`, GAP-6/GAP-7; PR #477 plan). No schema change is needed (`CommunicationLog` carries `externalId` + `status SENT/FAILED/BOUNCED`; `User.newsletterConsent` / `marketingConsentAt` already exist). Enablement is the owner env flip (`RESEND_API_KEY`, `EMAIL_SANDBOX*`, `EMAIL_GLOBAL_DAILY_CAP`, then `EMAIL_ENABLED=true`) per the transactional-email enablement plan.
...
- [ ] **`EMAIL_GLOBAL_DAILY_CAP` sized with real launch headroom** (the GAP-1 global daily cost gate; env-driven, never relaxed in prod). Treat a gate trip as a **paging / ops signal**, not a routine limit: gate exhaustion blocks ALL transactional email **including admin OTP**, so an under-sized cap can lock field/admin users out of login (Codex ops note, PR #512). The GAP-6 auto send-pause escalates repeated gate trips (and a bounce-ratio spike) into a hard pause that only a SUPER_ADMIN can clear via the email-ops route.
