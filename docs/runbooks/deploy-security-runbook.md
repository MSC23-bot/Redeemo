...
- **Migrations:** run **`npx prisma migrate deploy`** against the environment's DB **before** the new code serves traffic (it applies committed migrations, never generates new ones). The **`Procfile` intentionally has NO `release:` line** — migrations are kept as an explicit, documented pre-deploy step for portability + control (not every platform runs a Heroku-style release phase, and an auto-applied migration is harder to gate / roll back). A platform that **does** support a pre-deploy/release phase **may** run `npx prisma migrate deploy` there instead; otherwise run it as a manual pre-deploy step (see §4). The two Phase-0 migrations (`CommunicationStatus += QUEUED` + `BranchPhoto` moderation fields) are **additive** — a code rollback leaves harmless extra columns/values, no data loss (§10).
