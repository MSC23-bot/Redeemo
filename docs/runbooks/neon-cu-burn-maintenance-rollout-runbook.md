...
0. **Provider pre-deploy migration hook disabled FIRST (§3.1 E7).** Migrations for this activation run **ONLY ONCE, from the controlled operator process on the verified DIRECT endpoint** (step 1 below). The Railway Web pre-deploy `npx prisma migrate deploy` inherits the Web service's runtime `DATABASE_URL` — recorded as **pooled** (r1 §2) — so any subsequent Web deployment with that hook active would **rerun Prisma migration over a prohibited pooled path**, re-entering the failure territory r1 §13 records (the prior pre-deploy migrate failed with P1001; the pooled host is a confirmed migration mismatch, though not proven to be the P1001 cause) and violating this section's pooled-migrations prohibition. E7 must therefore be verified DONE before step 4 below and re-verified at §6 step 6. **Railway Web must not run any pooled migration during the subsequent deployment.** (Web-only: the **worker** service carries no pre-deploy migrate hook — the Procfile has no release line and §6 step 9 is a daemon start, not a migration.)
