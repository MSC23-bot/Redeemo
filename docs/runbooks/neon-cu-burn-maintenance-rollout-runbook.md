...
> **STATUS: DRAFT / PREPARATION ONLY — do NOT execute.** Docs-only. No step here has been run: no provider was accessed, no variable set, no migration applied, no deployment or restart triggered, no Redis key inspected or deleted while drafting this document. Execution requires Codex review + explicit SHA-bound owner approval, and remains gated by the R1 recovery preconditions (**P1a PASSED + P8 ESTABLISHED 2026-07-03; P1b and P9 remain BLOCKED/OPEN**, `docs/runbooks/r1-key-rotation-activation-runbook.md` §1/§13 + §13.3.1 + §13.5.1).
>
...
| Web service | **SERVING the pre-R1 recovery baseline** since 2026-07-03 (deployment `6d26b0b4…` Active; source = `recovery/pre-r1-baseline` @ `53bafac4…`; pre-deploy migration hook ABSENT — r1 §13.5.1). Previous state: Failed on `1d65f2ec…` | Confirm current deployment state + history; re-verify the visible Redeploy action (P8 retention rule) |
...
| R1 gates | **P1a PASSED + P8 ESTABLISHED 2026-07-03 (r1 §13.3.1 + §13.5.1); P1b and P9 remain BLOCKED/OPEN; D-R10 OPEN** | The remaining gates block execution of this runbook (see §6 step 1); P8's visible provider action must be re-verified (retention rule) |
| Neon spending limit | Owner-set `$20` known | Hard-stop enforcement remains **UNVERIFIED** (§8) |

**Plainly: while P1b and P9 remain blocked (P1a PASSED + P8 ESTABLISHED 2026-07-03), NO step of §4-§7 may execute.** The r1 runbook §13.8 recovery ordering (compute headroom → resume → P1a → pre-R1 baseline + P8 → P9 fixture → P1b → migration) must complete first or alongside; this runbook's rollout sequence (§6) states where it slots in.
...
| 2 | **Record known-good rollback evidence** — the protected pre-R1 branch (`53bafac4…`) rebuild path + a currently-visible Railway Redeploy/Rollback action (r1 P8/D-R6 BOTH-legs model) — **established 2026-07-03 (r1 §13.5.1); RE-VERIFY the visible action at execution time (retention rule)** | §13.5 evidence recorded + re-verified | Either evidence leg missing ⇒ P8 BLOCKED |
...
1. **P1b and P9 are BLOCKED/OPEN (P1a PASSED + P8 ESTABLISHED 2026-07-03 — r1 §13.3.1 + §13.5.1)** — this runbook is not executable until the r1 §13.8 recovery ordering closes the remaining gates.
