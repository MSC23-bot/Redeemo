...
| 5 | **Apply the additive migrations ONCE** from the controlled operator process via the verified DIRECT endpoint (§4.2). The Web pre-deploy migration command should already be absent (removed by the recovery deployment, r1 §13.8 step 4); migrations ALWAYS precede the step-6 Deploy that ships the new image | `migrate status` → up to date; expected set matched exactly | §4.2 stop conditions |
| 6 | **Deliberately deploy the merged Web image** per the r1 sequence (§13.8 step 7-8 / r1 §4) — auto-deploy stays disabled; `<R1_COMMIT>` re-anchored (§4.3 D-M2). **Before clicking Deploy: visually confirm the Web service's pre-deploy migration command is ABSENT** (§3.1 E7); if it is somehow still present, STAGE its removal into THIS deliberate deployment (never as a standalone save) and re-verify the staged set (E5). Also verify the staged set carries the source-branch restoration to `main`/R1 and nothing else (r1 §13.8 step 8). This single Deploy applies the staged configuration and runs **build + boot ONLY** against the already-migrated schema (step 5); Railway runs NO migration | Build+boot success (no migration executed by the provider); `/health` 200; `web` fingerprint row present | **The staged set contains anything unexpected ⇒ Discard + STOP**; a migration command or pooled migration path would remain active after this Deploy ⇒ STOP before deploying; r1 §4 failure paths (rollback to the P8 target) |
