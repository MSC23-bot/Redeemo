...
| 5 | **Apply the additive migrations** via the verified DIRECT endpoint (§4.2) | `migrate status` → up to date; expected set matched exactly | §4.2 stop conditions |
| 6 | **Deliberately deploy the merged Web image** per the r1 sequence (§13.8 step 7-8 / r1 §4) — auto-deploy stays disabled; `<R1_COMMIT>` re-anchored (§4.3 D-M2) | Build+boot success; `/health` 200; `web` fingerprint row present | r1 §4 failure paths (rollback to the P8 target) |
...
| L4 — provider state | Restore provider source/variables | Only through the separately-approved provider steps (r1 §13.4 model); auto-deploy stays disabled | Never silently |
