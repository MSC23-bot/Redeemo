...
>
> **Freshness:** Business Profile day-2 v1 (#393-#396, `origin/main` @ `ade3efe5`) verified + folded into §4.2 on **2026-07-06**; the prior Merchant re-verification was @ `a58db583` (2026-07-05). Other programmes last verified @ `434ca4eb` (2026-06-28) and were **not** reverified this pass. **Known lag (still not folded into §4.2): PRs #375-#392 (staff error-surface pins, #377 PIN wire hygiene, D-BM1 #379/#381, smoke journeys #378/#382-#384, #385 security-runbook reconciliation, #386-#390 session cache-isolation wave, #391 docs-architecture migration, #392 transport epoch re-gate) postdate the `a58db583` stamp - evidence in git + Codex Vol-2; next Merchant re-reconciliation must absorb them.** Lines without
...
- **Merged:** `origin/main` @ `ade3efe5` (latest merge: Business Profile M4 #396; the intervening #375-#392 wave is in git but only Business Profile v1 is folded into §4.2 - see the freshness header). Verify any "merged" claim with `git log`/the PR.
...
**Gated / not-done:** Insights behavioural + event CSV = **LEGAL-GATED** (default-off, fail-closed); Insights demographics = **LEGAL-GATED + NOT-STARTED**; Home Live dashboard = **PLACEHOLDER/OWNER-GATED**; Home Staff view = **NOT-STARTED/OWNER-GATED**; Business Profile (day-2 settings) v1 = **COMPLETE** (M1-M4 merged 2026-07-06; see the Business Profile line above); its Documents (§BP-DOC) + My Account (§BP-ACC) sub-surfaces = **NOT-STARTED/deferred**; My Account, Help & Support = **NOT-STARTED**; Promote, Payments & Billing = **NOT-STARTED/PROVIDER-GATED**; global-shell gaps CLOSED by #364 (logout-confirm, active-route highlight, notification deep-links, Quick-Actions launcher placement, responsive collapse/tab bar, favicon); View-as lens NOT built - the prototype itself labels it "Prototype control only. Not part of the live portal." (owner to record the drop; same for the Demo switcher).
...
| D67: read-only admin visibility of voucher redemptions REQUIRED before pre-launch staging acceptance can pass (owner-flagged 2026-07-05/06; also recorded in `docs/runbooks/deploy-security-runbook.md` + the 2026-07-05 reconciliation runbook) | Admin/platform | staging-acceptance blocker | pre-launch |
...
- **Admin Panel prototype:** `docs/superpowers/specs/2026-07-01-admin-panel-platform-blueprint.md` + `docs/design/admin-panel/upload-bundle/`; live execution log `docs/design/admin-panel/prototype-execution-log.md` (untracked); open reconciliation PR #351.
