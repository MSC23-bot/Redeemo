...
**In review (open PRs, NOT merged):** PR #365 (PR-G1b deterministic local Playwright smoke lane, advisory CI job; head `410b0c67` after the Codex safety round: no foreign-server reuse, forbidOnly, adjudicated comments, paging-accurate mock) and PR #366 (Vouchers V1 builder parity; head `cf6d510c` after the Codex contract round: expiryDate ISO normalization, imageUrl/expiryDate hydration, honestly-constrained saved photo AND end-date removal, FileUpload interaction pins) - awaiting SHA-bound owner/Codex merge approval. Authenticated staging acceptance remains OUTSTANDING for every Merchant module.
