...
>
> **Freshness:** Merchant rows re-verified against `origin/main` @ `1eb2b382` on **2026-07-04** (through the merged #364/#365/#366); other rows last verified @ `434ca4eb` (2026-06-28).
...
- **PR-G1b** - MERGED (#365, squash `cda007f7`, 2026-07-04): deterministic **local** Playwright smoke (`@playwright/test` 1.61.1, owner-approved) against a locally-built merchant-web with an enforced dead-port + route-mock boundary; 17 tests incl. the #324/#327 regression classes. CI job `merchant-web-smoke` is **ADVISORY** - promotion to a required check remains a separate owner decision (T2 rule unchanged).
