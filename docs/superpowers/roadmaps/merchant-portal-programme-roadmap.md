...
>
> **Freshness:** shell/nav rows re-verified against `origin/main` @ `8c4258ba` on **2026-07-04** (the merged shell wave #364); other rows last verified @ `434ca4eb` (2026-06-28).
...
| **Sidebar nav** | active-route highlight; lifecycle/capability gating | `Sidebar.tsx`, `navItems.ts`; role positive-allowlist (least-privilege baseline for null/unknown/STAFF) + Insights capability-gated | `/merchant/profile.viewerCapabilities` (+role/canManageVouchers/displayName, #364) | MERGED / NOT-ACCEPTED (#364) | - (highlight live; 0 dead links; 72px collapse + narrow bottom tab bar shipped) | - | sidebar/mobile-tab-bar/shell jest + Playwright smoke (PR #365, in review) | screen DoD |
| **Topbar: Validate-a-code** | scan/enter → validate; CTA enabled in EVERY lifecycle incl. suspended (in-dialog "Validation paused" copy); label icon-only when compact | `Topbar` + `ValidateDialogProvider` (#364: label collapses < 720px; suspended stays ENABLED - intentional prototype behaviour, the pause is explained inside the dialog, NOT an unresolved gap) | `/merchant/redemption/verify` (JWT+DB, #328) | MERGED / NOT-ACCEPTED (#364) | - | code sensitive; authz ok | unit + smoke (PR #365, in review) | screen DoD |
| **Topbar: Quick-Actions / app launcher** | popover incl. **branch-PIN reveal** sub-step | `QuickActionsMenu` (#364): Validate / Create voucher (capability) / branch-PIN ROUTES to the guarded on-page PinCard via `/branches/{id}#pin` (owner-only this wave; never inline) / Today's redemptions | PIN reveal: `GET /merchant/branches/:id/pin` (guarded, reused) | MERGED / NOT-ACCEPTED (#364) | extend PIN row to assigned BMs when PinCard gains a per-branch capability signal | PIN never rendered in the popover (test-pinned) | quick-actions jest + Playwright smoke (PR #365, in review) | screen DoD |
| **Account dropdown** | My account / Business profile / Help / identity / **logout confirm** | full menu (#364): identity header (business + person - role), destinations, §13.4 logout CONFIRM dialog; Business profile OWNER/BM allowlist | session signOut | MERGED / NOT-ACCEPTED (#364) | - | shared-device safety shipped | topbar jest + Playwright smoke (PR #365, in review) | screen DoD |
...
| **View-as role lens (topbar)** | Owner views portal as BM/Staff | absent - the prototype itself footnotes the control "Prototype control only. Not part of the live portal." (same for the Demo switcher) | - | DROP RECOMMENDED (owner to record; prototype text resolves the conflict) | - | - | - | owner record |
...
| **Redemptions log + CSV** | merchant-wide log, filters, CSV, confirm-before-validate, detail drawer | `(app)/redemptions/*` (feature-complete against the LOCKED M3 spec; "NOT-ACCEPTED" means the DoD staging-acceptance gate is outstanding, NOT a functional shortfall - 2026-07-04 discovery) | `src/api/merchant/redemptions/*` | MERGED / NOT-ACCEPTED (staging gate) | small fidelity items only (voucher-instance filter UI, search scope/label, sort control - S each) | IDOR-safe, PII-redacted, no-PIN select; #328 dual-auth | 6 backend suites (**integration not in CI**) | module DoD |
...
- **2026-07-04** - Shell wave #364 MERGED (squash `8c4258ba`): sidebar/topbar/account/notifications/quick-actions/coming-soon rows flipped to MERGED / NOT-ACCEPTED; View-as row moved to DROP-RECOMMENDED per the prototype's own "Prototype control only" footnote; notifications page-vs-overlay stays the open owner divergence. In-review pointers: PR #365 (G1b smoke lane, `3f9bf74e`), PR #366 (Vouchers V1 builder parity, `f829ed46`).
