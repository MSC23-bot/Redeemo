...
- **Explicitly DEFERRED on the detail/list (recorded in §10):** the PROMINENT big-stat per-row redemption block from the prototype (the shipped list DOES render an inline per-row count - `VoucherCard` `"{n} redemption(s)"` - this line previously overstated the deferral; corrected 2026-07-04); per-voucher analytics/charts/trends/branch-breakdown/validated-rate; the live-voucher "request a change" / "request to end" / "run again" row actions; the prototype `ended`-vs-`expired` and `end_review`/`changes_review` sub-states beyond what (status, approvalStatus) can represent.
...
- **Editable `adminProposed` field set:** `title, description, terms, estimatedSaving` (plus admin `imageUrl`). NOTE (2026-07-04 reconciliation): `availabilityWindows` / `cooldownSeconds` were listed here aspirationally but PR-A shipped WITHOUT them - the backend `buildAdminProposed` allow-list covers the string fields + estimatedSaving only. The structured windows/cooldown concierge corrections are a RECORDED owner-gated deferral (baseline memory), not shipped behaviour. Values are only ever applied through the validated `updateVoucher` path (type-guarded).
