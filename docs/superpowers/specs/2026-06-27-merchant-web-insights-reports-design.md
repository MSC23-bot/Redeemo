...
Owner-facing labels and exact half-open Europe/London implementation intervals: **Overnight `[00:00, 07:00)` · Morning `[07:00, 12:00)` · Lunch `[12:00, 15:00)` · Afternoon `[15:00, 18:00)` · Evening `[18:00, 22:00)` · Late `[22:00, 24:00)`**. No gaps/overlaps; no activity dropped/folded; after-midnight belongs to its **actual London calendar day** (not the previous evening); described as "when activity was **logged**"; active filters + server scope; six-column layout responsive without removing buckets; **sparse-cell treatment is a bounded-review (PR-0a) output** (exact-vs-intensity-only-vs-hidden; minimum cell threshold; the **"Busiest" badge threshold**; anti-inference across filters). **Until that output exists, busy-times uses a documented safe fallback** - **server-side** suppression defaulting to **intensity-only with no exact sparse counts** (raw counts and peak info never reach the browser when suppressed) - or is unavailable. Tooltip explains hours, timezone, logged meaning, after-midnight behaviour, and privacy. A **"Busiest: <day> <daypart>" badge** + highlighted peak cell is computed from logged activity and **omitted** when the peak cell is below the (bounded-review) threshold rather than naming a near-empty peak. Route/payload tests prove suppression cannot be bypassed through filter combinations.
...
| Setting up / Submitted / In review / Changes needed (pre-live) | Lifecycle-**locked** "unlocks when you go live" - and **server-blocked** (Insights requires `Merchant.status==='ACTIVE'`, §7.2), not just UI-hidden |
| Suspended | The existing **suspension screen** (SEC-M2 hard-block); **no Insights data is retrieved** (the resolver blocks SUSPENDED). Historical data is preserved in the DB but not exposed through Insights. |
...
|---|---|---|---|
| Access Insights | Yes - all merchant branches (`allBranches`) | Yes - **all merchant branches if `allBranches=true`, else only `allowedBranchIds`** (empty specific set → no data) | No - server-side deny; nav excludes |
...
- **Active-merchant gate (SEC-M2 preserved):** every `/insights/*` endpoint requires **`Merchant.status === 'ACTIVE'`** before any query. `resolveMerchantContext` already hard-blocks SUSPENDED (`MERCHANT_SUSPENDED`); the explicit `ACTIVE` check additionally blocks REGISTERED / PENDING_APPROVAL / INACTIVE / DELETED, so **pre-live merchants are server-blocked, not just UI-hidden**. Suspended merchants keep the suspension screen; historical data is preserved but never exposed through Insights. No bypassing read-only resolver is created. Server tests cover all six statuses (§7.3).
...
| Merchant `status === 'ACTIVE'` | Insights available (subject to all other authz + the runtime gate) |
| Merchant status REGISTERED / PENDING_APPROVAL / INACTIVE / SUSPENDED / DELETED | **no Insights data** (SUSPENDED via the suspension screen; the others server-blocked) |

Every case is exercised on each `/insights/*` endpoint **and** each export, with live re-resolution. The six merchant statuses each get a server-side test.
...
- **Lifecycle authorization (SEC-M2 preserved, §7.2):** Insights requires **`Merchant.status === 'ACTIVE'`** server-side **before any query**; REGISTERED / PENDING_APPROVAL / INACTIVE / SUSPENDED / DELETED return **no Insights data** at the server (pre-live merchants are server-blocked, not just UI-hidden); frontend hiding is **never** the authorization boundary; server tests cover all six statuses.
- **Pre-live:** lifecycle-locked "unlocks when you go live" (UI) **and** server-blocked (above).
...
- **Suspended:** the existing **suspension screen** (SEC-M2 hard-block); **no Insights data retrieved**; historical data preserved but not exposed. No bypassing read-only resolver is created.
...
- `GET …/insights/overview` → **redemption activity** as `{ logged, confirmed, awaiting }`; **distinct customers** as a single logged figure; **repeat-customer rate** as a percentage over logged history (with its own insufficient-history flag); **each KPI - and the savings block - carries its OWN comparison object** (null on incomplete periods; repeat-rate's value AND its comparison are behind the behavioural gate, collapsing to `{available:false}` when closed; never a single shared comparison object; final per-KPI shape in the plan §2.7) + savings block (estimated logged / estimated confirmed / awaiting) + `meta` (effective scope, earliest date for All-time, filters echoed).
...
### 22.6 Codex re-review of the implementation plan (lifecycle + busy-times spec corrections)
Two spec corrections were made alongside the implementation-plan amendments (PR #330 round), driven by Codex's plan review + an owner lifecycle decision:
1. **Lifecycle authorization (owner decision A - preserve SEC-M2):** the earlier "suspended sees the established dashboard read-only" was unachievable - `resolveMerchantContext` hard-throws `MERCHANT_SUSPENDED` (`shared.ts:136`) and login blocks SUSPENDED, while pre-live statuses were not server-blocked by the resolver. Corrected: Insights requires **`Merchant.status === 'ACTIVE'`** server-side before any query; SUSPENDED keeps the suspension screen (no data; no bypassing read-only resolver); REGISTERED/PENDING_APPROVAL/INACTIVE/DELETED are server-blocked (not just UI-hidden); six-status server tests; frontend hiding is never the boundary. §2.1, §7.2, §7.3, §11.
2. **Busy-times sparse-cell:** the exact-vs-intensity-vs-hidden treatment + the "Busiest" badge threshold + anti-inference are a bounded-review (PR-0a) output; until then a server-side **safe fallback** (intensity-only, no exact sparse counts, raw counts/peak never reaching the browser) or unavailable; route/payload tests prove suppression cannot be bypassed via filters. §1.7.

A subsequent six-lens self-review of the amended plan + spec drove three further consistency corrections to this spec (closed scope; no behaviour change):
