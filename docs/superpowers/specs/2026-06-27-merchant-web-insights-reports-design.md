...
- **Reusable authz spine:** `resolveMerchantContext(prisma, adminId)` → `{ adminId, merchantId, role(OWNER|BRANCH_MANAGER|STAFF), allBranches, allowedBranchIds, canManageVouchers }` (`src/api/merchant/shared.ts:124-142` - type + function), with live SEC-M2 suspended-merchant block; `assertBranchAllowed` (`shared.ts:150-152`); the `scopeBranchIds(ctx)` pattern (`src/api/merchant/redemptions/routes.ts:11-13`, returns `null`=all or `allowedBranchIds`, intersected server-side). **Insights inherits the resolver but NOT `scopeBranchIds` verbatim** - the existing `scopeBranchIds` returns `null` (merchant-wide) whenever `ctx.allBranches` is true, and `MerchantMembership.allBranches` **defaults to true**, so a `BRANCH_MANAGER` would leak merchant-wide data. Insights uses a **role-aware** scope (§7.2): merchant-wide only for `role==='OWNER' && allBranches`; a `BRANCH_MANAGER` is always restricted to `allowedBranchIds` (empty set → no data, fail-closed).
...
|---|---|---|---|
| Access Insights | ✅ authorised merchant-wide | ✅ **only `allowedBranchIds`** | ❌ (nav excludes; route unreachable) |
| Branch filter | "All branches" + any branch | only authorised; 1 → static "Viewing: <Branch>"; many → "All my branches" + authorised | - |
...
- **Role-aware branch scope (NOT the verbatim `scopeBranchIds`):** the effective scope is `null` (merchant-wide) **only** for `role==='OWNER' && allBranches`; a `BRANCH_MANAGER` is **always** restricted to `allowedBranchIds` and **never** receives merchant-wide data even though `allBranches` defaults to true; an empty `allowedBranchIds` → no data (fail-closed). The merchant-redemptions `scopeBranchIds` (which returns `null` on `allBranches` regardless of role) must **not** be reused as-is.
...
- The merchant-wide "All branches" option is **never** offered to a Branch Manager.
...
- `GET …/insights/report` (HTML) **or** a client-rendered print view fed by the above → §10.2. If server-rendered, it is a §15.3-governed endpoint (live authz re-resolution + Staff deny + role-aware scope + §4.1 cleanliness + HTML-injection-safe escaping).

### 15.3 Contract rules - [LOCKED]
- All endpoints: `resolveMerchantContext` + **server-side Staff deny** (`role!=='STAFF'`) + **role-aware branch scope** (§7.2: merchant-wide only for `OWNER && allBranches`; `BRANCH_MANAGER` always restricted to `allowedBranchIds`) intersected at the WHERE level; live re-resolution per request; filters parsed/validated server-side; **suppression server-side**; Decimal → `Number()`; London bucketing via **parameterised** SQL `DATE_TRUNC(... AT TIME ZONE 'Europe/London')`. Use Prisma's tagged-template **`$queryRaw`** (true parameterisation) for client-influenced values (date range, branch IDs), **not** `$queryRawUnsafe`; the concrete precedent for the VoucherRedemption→Branch→User raw GROUP BY is `src/api/customer/discovery/homeRailBuilders.ts:computePopularityScores`. The **QA-account exclusion** is `isQaAccountEmail` (a code helper, **not** a relational column), so it is resolved to a parameterised SQL predicate over `user.email` (e.g. `email NOT IN (:qaEmails)` + domain `NOT ILIKE` built from compile-time constants) - see §4.1.
...
**Deferred [DEFER]:** demographics until all gates clear (§14); reversal schema + "reversed-excluded" wiring (Phase-4); QR-vs-manual usefulness until the mobile app emits `QR_SCAN`; server-side PDF + email report delivery; retention-enforcement job (if adopted); rollups/caching unless evidence requires; any inferred sensitive/special-category analytics; customer-interests analytics.

**Stop-and-report triggers (hard):** any schema/migration (analytics index, future reversal columns, a possible Gender enum); any **customer-facing or cross-product contract change**; any **legal-copy/policy** change (handled only in the PR-0a→artefacts→PR-0b flow); any provider/email enablement; any production-infrastructure or rollup change; any query that cannot be served safely at production volume (no silent truncation); any inferred special-category analytics.
...
1. **(Security) Role-aware branch scope** - a `BRANCH_MANAGER` whose `allBranches` defaults true would leak merchant-wide via the verbatim `scopeBranchIds`. Fixed: §3.1, §7.2, §1.9, §15.3 - Insights uses a role-aware scope (merchant-wide only for `OWNER && allBranches`; `BRANCH_MANAGER` always restricted to `allowedBranchIds`, fail-closed on empty).
...
Grep sweep confirms **no surviving stale wording**: every "validated-only" is in the superseded §1.0 baseline or a prototype-divergence row describing what is replaced; every "delivered" is a "never/dropped" rule, the language ban, or the prototype's own "Value delivered" label being replaced; "no personal data ever / personally identifying" appears only as the rule forbidding it and the verbatim quote of the existing privacy policy (§13.2); every demographic "placeholder/coming soon" mention is the rule forbidding them; every "merchant-wide" + Branch-Manager mention is the corrected "never merchant-wide for a BM" / role-aware rule (Owner merchant-wide is correct). No must-fix or clearly-correct should-fix remains open.
