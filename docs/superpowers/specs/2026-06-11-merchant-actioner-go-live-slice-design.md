...
- **Migration (expand → backfill → reroute → contract, safe because only seed/test merchants exist):** add `MerchantMembership` + `MerchantMembershipBranch` + `MerchantRole`; **backfill** one `OWNER` membership (`allBranches=true`, status=ACTIVE) per existing `MerchantAdmin`; switch `resolveAdminMerchant` and the merchant-**management** reads to resolve **admin → OWNER membership → merchantId**. The **contract step — drop `MerchantAdmin.merchantId @unique` + the `merchant` relation — is sequenced LAST, AFTER the merchant auth + branch-user paths that read `admin.merchantId` / `include: { merchant: true }` directly are rerouted through membership** (those reads pair with the SEC-M2 live-status auth rework, §7). The expand + backfill + management reroute ship first as a **purely additive** step; `MerchantAdmin.merchantId` is **kept as a transitional compatibility field** until the auth reroute lands. (D-1: RESOLVED — the old 1:1 field IS removed; the drop is sequenced after the auth reroute, not in the first additive step. Verified during implementation that the auth/branch-user flows read the column + relation directly.)
...
- **D-1 contract step (lands here):** because SEC-M2 already reworks the auth flow's `admin.merchant.status` reads, this is where the remaining direct reads of `admin.merchantId` / `include: { merchant: true }` (login / OTP / login-complete / refresh / reactivate + branch-user mgmt) are rerouted through `MerchantMembership`, after which `MerchantAdmin.merchantId` + the `merchant` relation are dropped (the §3 contract step). The additive membership foundation + backfill ship earlier; this is the sequenced drop.
...
- **Expand → backfill → reroute → contract (D-1):** backfill one OWNER `MerchantMembership` per existing `MerchantAdmin`, reroute `resolveAdminMerchant` + the merchant-management reads through the membership (additive; `MerchantAdmin.merchantId` **kept as a transitional field**). The **drop of `MerchantAdmin.merchantId`** (+ its `@unique` + the `merchant` relation) is **sequenced after the merchant auth + branch-user reads are rerouted** (§3, §7) — safe, seed/test-only data, no real merchants exist.
...
All migrations applied via `prisma migrate deploy` (the membership foundation is additive; the one safe column drop is **sequenced after the auth reroute**, not in the first additive step; no data loss — no real merchants exist).
...
- **D-1 — remove `MerchantAdmin.merchantId` (the old 1:1 field): YES; drop sequenced after the auth reroute.** Clean `MerchantMembership` foundation; backfill OWNER memberships + reroute resolution as a **purely additive** step first (keeping the column as a transitional compatibility field), then drop the old 1:1 field + relation once the merchant auth (login/OTP/refresh/reactivate) + branch-user paths that read it **directly** are rerouted through membership (they pair with the SEC-M2 auth rework). **End-state unchanged: the 1:1 field is removed.** (§3, §7, §13) *(Sequencing corrected during implementation after the blast-radius map showed the auth/branch-user flow reads the column + `admin.merchant` relation directly, not via `resolveAdminMerchant`.)* **✅ SHIPPED in M6b (2026-06-12): `MerchantAdmin.merchantId` + its `@unique`, the `MerchantAdmin.merchant` relation, and the reverse `Merchant.admin` are dropped (migration `20260612014222_drop_merchant_admin_merchantid`, guarded by an "every admin has an ACTIVE OWNER membership" pre-flight check). `MerchantMembership` is now the SOLE admin → merchant source of truth.**
...
- **Expand-contract drop (D-1), sequenced late.** Dropping `MerchantAdmin.merchantId` is a non-additive migration; safe *only* because data is seed/test-only. **The merchant auth + branch-user flows read `admin.merchantId` + the required `admin.merchant` relation directly (not via `resolveAdminMerchant`)** — so the drop must come AFTER those reads are rerouted through membership, which is why it is sequenced with the SEC-M2 auth rework (§7) rather than in the first additive membership step. *Mitigation:* ship expand + backfill + management-path reroute additively first (column kept as a transitional field); then reroute the auth/branch-user reads, assert every `MerchantAdmin` has an OWNER membership, and drop — no production data to lose. If real merchants somehow existed, this would need a staged expand→verify→contract across deploys.
