...
> Self-review: completed via a fresh 5-lens adversarial review; verdict + integrated fixes recorded in Section 13.
...
1. Lists every branch of their merchant as cards, scoped to what their role/branch assignment allows them to see. The overview carries summary cards (Locations count; Open-now count; With-Redeemo / in-review count) and per-row columns (name, status open/closed, locality/town, Today's hours, a Setup column = PIN status + amenity count, a main-branch indicator).
...
| 10 | Staff & Access membership data via `GET /merchant/staff` (`listMembers`, returns `branchIds[]`; OWNER-GATED via `assertOwner`). `getBranch` does NOT return staff. | Staff-at-branch display | PR-1 (owner-only display; see PR-1 notes) | no |
...
- Branches list (prototype `01`/`12`): summary cards (Locations count; Open-now count; With-Redeemo / in-review count) + branch rows with columns (name; status open/closed; locality/town; Today's hours; Setup = PIN status + amenity count; main-branch indicator), scoped to what the viewer is allowed to see (owner/allBranches see all; a scoped member sees only allowed branches, via the existing scoped `listBranches`). A disabled "Add branch" affordance is visible (locked, PR-5).
- Branch detail page sections:
  ...
  - Staff at this branch (prototype `05`/`06`): DISPLAY ONLY, and OWNER-ONLY-VISIBLE in PR-1. The data source is `GET /merchant/staff` (`listMembers`, which returns each member's `branchIds[]`), filtered client-side by this branch; it is NOT on `getBranch`. `GET /merchant/staff` is owner-gated (`assertOwner`) today, so a Branch Manager cannot load it: PR-1 therefore hides the staff-at-branch section for non-owners. Display members with their access-type pill (Portal+app / App access). The "Assign or manage staff" cross-link routes to the existing Staff & Access surface and is owner-gated (D3). PR-1 introduces NO new staff write and MUST NOT relax `assertOwner` to make staff-at-branch BM-visible (that is a stop-and-report / PR-2 item).
...
Test expectations (PR-1): merchant-web component/integration tests for the list (scoped rendering + summary cards + setup column), detail sections, owner instant-save contact/amenities/PIN, the owner edit-request submit + pending display + withdraw, the owner-only set-main asymmetry (badge vs button), staff-at-branch owner-only visibility, photo gallery display-only, and that every locked affordance renders DISABLED and performs no network write. A non-owner Branch Manager sees reads only and no write controls. `tsc` + lint + jest green on `apps/merchant-web`.
...
2. Owner-only actions (create, close, set-main, staff/role management, business-wide settings) are server-rejected for non-owners; staff-at-branch read is owner-gated today and must not be relaxed to make it BM-visible without a deliberate PR-2 decision.
...
7. Redemption-PIN values stay AES-encrypted at rest and are never leaked into list/notification payloads.
...
- The prototype screenshot set is the binding visual reference; where this spec's prose and the screenshots disagree on layout/affordance, the screenshots win for visuals and this spec wins for governance/behaviour; reconcile at the slice plan.
...
- Staff-at-branch is not on `getBranch` and `GET /staff` is owner-gated: PR-1 now sources it from `GET /merchant/staff` filtered client-side, owner-only-visible, with a stop-and-report against relaxing `assertOwner` (PR-1 staff bullet + row 10 + Section 8 #2 + stop-and-report).
...
Verdict: the spec is internally consistent, faithful to D1-D9 and the prototype, grounded in the verified live code, and safe for a low-owner-input background implementation of PR-1 (with PR-2 to PR-8 contextually locked and each later schema/governance slice deferred to its own mini-spec). Ready for owner review + Codex review. No implementation plan or code until both approve.
