...
- Branch reuse: the Add main branch step offers to reuse the business profile head office address, company website, and description so a single-location merchant does not retype them. Opening hours support multiple periods per day (split shifts) plus an Open 24 hours option. Branch-specific fields confirmed against the customer profile: name, address, customer phone, branch website, branch description, opening hours, amenities or features, photos (distance, directions, open status, and reviews are derived or system).
