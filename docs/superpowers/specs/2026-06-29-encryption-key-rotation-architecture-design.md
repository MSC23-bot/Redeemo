...
| # | Correction | Where |
|---|---|---|
| R5-#1 | Split the single P1 gate into **P1a (runtime/pooled, serve only)** + **P1b (direct/migration, before any schema mutation)**; minimum read-only preflight permitted; first migration = execution confirmation, not P1b proof | §20.1; runbook §13.1–§13.3 |
| R5-#2 | **`MIGRATION_DATABASE_URL`** recommended (migrate-direct, runtime-pooled), fail-closed validation + fallback; bounded change w/ tests/secret/rollout/rollback/approval; permanent auto-migrate removal is a separate policy decision | §20.2; runbook §13.6; plan "Pre-R1 staging recovery" |
| R5-#3 | **P8 = protected recovery branch** at the verified pre-R1 SHA + serving evidence (`/health` + read-only categories smoke) + durable rebuild; image rollback is retention-limited; re-establish P8 if it disappears | §20.3; runbook §13.4–§13.5 |
