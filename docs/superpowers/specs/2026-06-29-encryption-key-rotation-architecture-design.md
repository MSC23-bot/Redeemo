...
interface KeyProvider {
  ...
  getLegacyKid(): string
  listKids(ns: 'pin' | 'otp'): string[]                 // verify / telemetry
}
```
...
| `OTP_HMAC_KEY_ACTIVE` | active OTP kid |
| `DECOMMISSIONED_KIDS` | comma-separated denylist of kids that can NEVER be ACTIVE (hard-blocks pasting the leaked key back as active) |
...
- **Two boot modes, selected PER-NAMESPACE (Amendment #4, refined by the consistency review — resolves the `ACTIVE=legacy` contradiction AND the R2-before-R4 trap).** Each ring's mode is selected **independently** by whether *its own* map var is set — the **PIN** ring is bridged iff `ENCRYPTION_KEYS` is unset; the **OTP** ring is bridged iff `OTP_HMAC_KEYS` is unset. This is critical because R2 (OTP separation) ships BEFORE R4 (PIN keyring): in the reachable intermediate state (`OTP_HMAC_KEYS` set with a fresh active value, `ENCRYPTION_KEYS` still unset) a single global toggle would wrongly put OTP into bridge mode and synthesize the OTP ring from the **leaked** `ENCRYPTION_KEY`, ignoring the operator's fresh OTP ring and leaving the compromised key the OTP signer — defeating R2/Operation A. Per-namespace selection prevents that.
  ...
  - **(B) Explicit-keyring mode (per ring)** — that ring's map var is set. For the PIN ring, `ENCRYPTION_KEY_ACTIVE` MUST be a `pin-*` kid, present, not denylisted, and **MUST NOT equal `ENCRYPTION_LEGACY_KID`**; `ENCRYPTION_LEGACY_KID` must be present in the pin ring. For the OTP ring, `OTP_HMAC_KEY_ACTIVE` MUST be an `otp-*` kid, present, not denylisted.
...
- Dedicated OTP ring (`OTP_HMAC_KEYS` + `OTP_HMAC_KEY_ACTIVE`) with `otp-*` kids; cross-namespace lookups rejected. Replace all **7** `createHmac` sites with one helper `otpHmac(challenge, code)` built on `getOtpHmacKey(getActiveKid('otp'))` (byte-correct keying per CRYPTO-1, §3.3). Enforcement is an **allowlist** (only `keyring.ts` / the boot bridge may read `process.env.ENCRYPTION_KEY` / `ENCRYPTION_KEYS`), pinned by a CI test — stronger than a denylist grep that `const k = process.env.ENCRYPTION_KEY` trivially evades.
- **Cutover with a verify-only previous — ROUTINE ROTATION ONLY (MF4 + Amendment R3-#7):** in a **routine** OTP rotation `OTP_HMAC_KEYS` keeps BOTH the **fresh** active kid AND the previous value as a deprecated, verify-only `previous` kid for `≥` the longest HMAC-challenge TTL (24h, set by `EMAIL_VERIFY_TTL`; the login-OTP challenge is only 10 min), so in-flight codes verify; after the window a follow-up config change drops the previous. `otpHmac` signs under ACTIVE; verification tries ACTIVE then PREVIOUS. **This overlap applies ONLY to routine rotation. The compromised-key INCIDENT path (this staging case) does the OPPOSITE: it does NOT retain the leaked OTP key at all — outstanding challenges are invalidated immediately (staging is offline, no legitimate in-flight codes to protect), per §3.12/§12.** PIN rotation alone does **not** fix OTP forgeability — setting a fresh OTP active value is an explicit, enforced step in both tracks.
...
- Harden the Guard-10 try/catch (`redemption/service.ts:284–295`) into a **three-bucket** taxonomy (should-fix — the design introduces a new parse-error class): (a) `KEY_NOT_AVAILABLE`/keyring error → alert + fail closed **loudly**; (b) `ENVELOPE_PARSE`/format error ("Invalid encrypted value format") → alert (distinct data-integrity severity) + fail closed loudly; (c) genuine GCM auth-tag mismatch (real wrong-PIN) → silent fail as today. Without (b), a malformed value silently bricks that branch's redemptions with no signal. All three pinned in the R1 test (Guard-10 moved into R1 per Amendment #1).
...
|---|---|---|
| OTP previous-key carry | Keep previous OTP value as verify-only for ≥ the max TTL (24h) so in-flight codes verify (§3.6) | **Do NOT preserve** the leaked OTP value. Staging is **offline**, so there are no legitimate in-flight challenges to protect — **invalidate outstanding challenges immediately** (drop the leaked OTP value at once; no dual-verify window). |
...
- **Guard 10 (R1):** `KEY_NOT_AVAILABLE`/keyring error alerts + fails closed loudly while a genuine wrong-PIN stays silent-fail as today.
