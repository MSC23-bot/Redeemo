...
**20.2 Runtime-pooled vs migration-direct — `MIGRATION_DATABASE_URL` (recommended long-term).** Today both the Prisma CLI (`prisma.config.ts` → `datasource.url = process.env["DATABASE_URL"]`) and the app runtime (`PrismaPg({ connectionString: process.env.DATABASE_URL })` in `src/worker.ts` + `src/api/plugins/prisma.ts`) read the **same** `DATABASE_URL`; there is no `directUrl`. The recommended design adds an optional **`MIGRATION_DATABASE_URL`** consumed **only** by the migration/CLI path (`prisma.config.ts` reads `MIGRATION_DATABASE_URL ?? DATABASE_URL`), while **runtime keeps the pooled `DATABASE_URL`** — preserving pooled runtime connectivity AND direct migration access. **Fail-closed validation (migration context):** in **controlled staging/production migration contexts**, `MIGRATION_DATABASE_URL` **MUST be present** and **MUST validate as a direct, non-`-pooler`** URL — **missing or invalid fails closed** (the migration does not run). **Any fallback to `DATABASE_URL` is explicit local/test-only behavior**, never the staging/production migration path. `prisma.config.ts` governs the **Prisma CLI / migration** configuration, so this is a **migration-context** fail-closed — **not** an ordinary application boot failure (runtime app boot is unaffected, still on pooled `DATABASE_URL`). This is a **bounded** architecture/code/provider change requiring: spec/plan/runbook amendment; code/config + integration tests (migrate uses the migration URL; runtime unaffected); secret handling (the direct URL is a new injected secret, never printed); provider rollout; rollback (unset → fallback); and **separate owner approval.** **Permanently removing the automatic pre-deploy migration is a separate deployment-policy decision — not part of incident recovery;** the temporary recovery disable of auto-migrate is reversible and recovery-scoped only. **Rejected:** repointing the runtime `DATABASE_URL` to the direct host (breaks legitimate runtime pooling).
...
| R5-#2 | **`MIGRATION_DATABASE_URL`** recommended (migrate-direct, runtime-pooled); **fail-closed in controlled staging/production migration contexts — MUST be present + direct/non-`-pooler` or fail closed; fallback to `DATABASE_URL` is local/test-only**; `prisma.config.ts` = Prisma CLI/migration config (not app boot); bounded change w/ tests/secret/rollout/rollback/approval; permanent auto-migrate removal is a separate policy decision | §20.2; runbook §13.6; plan "Pre-R1 staging recovery" |
