...
- **Cutover with a verify-only previous — ROUTINE ROTATION ONLY, explicit + provider-owned (MF4 + Amendment R3-#7 + R2-#1 owner-lock):** in a **routine** OTP rotation the operator sets `OTP_HMAC_KEY_ACTIVE` to the **fresh** kid AND `OTP_HMAC_KEY_PREVIOUS` to the **single** outgoing kid (both `otp-*`, both present in `OTP_HMAC_KEYS`, distinct, neither denylisted — §3.4). `getOtpVerifyKids()` then returns `[ACTIVE, PREVIOUS]`, so in-flight codes signed under the outgoing kid still verify. **Overlap-removal rule (LOCKED — Codex blocking #1; the overlap is a MINIMUM, not a cap):** a code signed an instant before cutover stays valid for the full longest challenge TTL (**24h**, `EMAIL_VERIFY_TTL`; the login-OTP challenge is only 10 min), so `OTP_HMAC_KEY_PREVIOUS` MUST remain configured until **AT LEAST `T_lastsign + 24h + Δ`**, where `T_lastsign` = the latest confirmed wall-clock time at which ANY Web or worker instance could still SIGN under the outgoing OTP key (i.e. the last instance finished rolling onto the fresh-`OTP_HMAC_KEY_ACTIVE` image — recorded by the operator from the deploy/rollout log), `24h` = the full longest challenge TTL, and `Δ` = the explicitly-bounded deployment/clock-skew allowance fixed by the runbook. PREVIOUS is removed **promptly after** that deadline — **not before** (removing earlier rejects a still-valid email-verification code) and **not left indefinitely** (every extra hour widens the verify surface). **Do NOT describe this as "at most 24h" / "≤24h"** — it is a minimum (`≥ T_lastsign + 24h + Δ`). After removal a follow-up config change **unsets `OTP_HMAC_KEY_PREVIOUS`** (and the retired kid is moved to `DECOMMISSIONED_KIDS`), returning verification to `[ACTIVE]`-only. PREVIOUS is **exactly one** kid, **never** "every non-active ring key". **This overlap applies ONLY to routine rotation. The compromised-key INCIDENT path (this staging case) does the OPPOSITE: `OTP_HMAC_KEY_PREVIOUS` stays UNSET, the leaked `otp-legacy` key is NOT accepted as a fallback, and outstanding staging OTP/email-verification challenges are invalidated immediately (staging is offline, no legitimate in-flight codes to protect), per §3.12/§12.** PIN rotation alone does **not** fix OTP forgeability — setting a fresh OTP active value is an explicit, enforced step in both tracks.
...
- **Parity fingerprint — ONE canonical formula (MF/should-fix; it was described three inconsistent ways in the draft).** The formula evolves by release via an explicit `codeCapability` bump (so an old image and a new image can never publish an identical fingerprint):
  ...
  // R2 (OTP key separation, Amendment R2-#2 — owner-locked) — DISTINCT capability + the ORDERED OTP verify list:
  fingerprint = sha256( JSON.stringify({
    codeCapability: 'v2-reader-otp-verify-v2',      // DISTINCT from R1's 'v2-reader-v1' — an R1 image vs an R2 image MUST mismatch even with identical keys/ACTIVE
    active: { pin: activePinKid, otp: activeOtpKid },
    otpVerify: getOtpVerifyKids(),                  // R2: the ORDERED list [ACTIVE] or [ACTIVE, PREVIOUS] — a different/unset PREVIOUS DESIGNATION MUST mismatch (NOT merely the unordered ring contents)
    keys: sortedBy(kid)([ { ns, kid, keyHash } ]),
  }) )
  ```
  The domain-separated `keyHash` (not a bare `sha256(keyBytes)`) avoids turning the "non-secret" fingerprint into an offline per-key confirmation oracle, and **the fingerprint still contains NO raw key bytes or HMAC material** (only domain-separated hashes + kid labels + the ordered verify-kid labels). **Including the active kids is required** — otherwise two services holding the same keys but a different ACTIVE would produce an identical fingerprint and silently pass the runner gate. **R2 additionally includes the ordered `getOtpVerifyKids()` list (Amendment R2-#2 — closes a real parity hole):** without it, two services with the SAME ring + SAME ACTIVE but a DIFFERENT (or unset-vs-set) `OTP_HMAC_KEY_PREVIOUS` would publish an IDENTICAL fingerprint yet accept different OTPs — the keyset is unordered and identical, so only the ordered verify-list captures the PREVIOUS designation. The **distinct R2 `codeCapability`** ensures an R1 image and an R2 image cannot publish the same fingerprint even with identical keys/ACTIVE (so R2-before-both-services-on-R2 cannot silently pass parity). The R1 formula is used by §3.11, §5, and the R1 test; the R2 formula supersedes it on R2 and is pinned by the R2 fingerprint tests (§11).
...
Never log plaintext PINs, key bytes, raw ciphertext, or full envelopes — anywhere in the migrator, decrypt-fallback telemetry, or counters. Only branch uuid + kid + masked host + counts. The keyring fingerprint uses the **single canonical formula defined in §3.9** — a domain-separated `keyHash` (`sha256('redeemo-keyring-v1:'+ns+':'+kid+':'+keyBytes)`) over the **release-specific** canonical object (R1 = `codeCapability: 'v2-reader-v1'` + active pin/otp kids + sorted `keyHash` set; **R2 additionally includes the ordered `otpVerify` = `getOtpVerifyKids()` list AND a distinct `codeCapability: 'v2-reader-otp-verify-v2'`** — so an R1 image vs an R2 image, and a PREVIOUS-designation divergence, both fail parity). **§3.9 is the sole complete source of the formula** — see it for the exact per-release objects (this paragraph does not restate them). The earlier stale `sha256(kid + sha256(keyBytes))` wording is removed (Amendment #10). The fingerprint never contains key bytes or HMAC material. Enforced by a **logging-redaction test**, not prose.
...
|---|---|---|
| OTP previous-key carry | Set `OTP_HMAC_KEY_PREVIOUS` to the single outgoing kid, kept verify-only until **AT LEAST `T_lastsign + 24h + Δ`** (the full longest challenge TTL after the last instance could still sign under it, plus the bounded deploy/clock-skew margin), then removed promptly — a MINIMUM, never "≤24h" (§3.6) | **`OTP_HMAC_KEY_PREVIOUS` stays UNSET.** Do NOT preserve the leaked OTP value. Staging is **offline**, so there are no legitimate in-flight challenges to protect — **invalidate outstanding challenges immediately** (`getOtpVerifyKids()` = `[ACTIVE]`; no dual-verify window; the leaked `otp-legacy` is never a fallback). |
...
| RISK-6 | OTP separation alone doesn't neutralise the leaked key for OTP; forgetting a fresh active value leaves OTP forgeable | Medium | R2 + Operation A make a fresh OTP value explicit + enforced; allowlist CI guard proves all 7 sites swapped. **Routine** rotation keeps the previous verify-only until AT LEAST `T_lastsign + 24h + Δ` (a minimum — the full longest challenge TTL after the last possible sign, plus the bounded deploy/clock-skew margin; never "≤24h") then drops it promptly; the **incident** track (this staging case) does NOT carry the leaked previous — challenges are invalidated immediately because staging is offline (§3.6/§3.12/§12) |
...
- **R2 explicit-previous-key (owner-locked) — REQUIRED tests:**
  ...
  - **R2 parity fingerprint (Amendment R2-#2, Codex blocking #2) — REQUIRED:**
    - identical ring + identical ACTIVE (pin+otp) + identical PREVIOUS designation + R2 capability ⇒ **identical** fingerprints (two services match).
    - same ring + same ACTIVE but a **different** `OTP_HMAC_KEY_PREVIOUS` (incl. set-vs-unset) ⇒ **different** fingerprints (the ordered `otpVerify` list differs) — so a PREVIOUS divergence fails Web/worker parity.
    - **R1 capability (`v2-reader-v1`) vs R2 capability (`v2-reader-otp-verify-v2`) ⇒ different** fingerprints even with identical keys/ACTIVE (an R1 image and an R2 image can never pass parity as compatible).
    - the fingerprint contains **no raw key bytes and no HMAC material** (only domain-separated `keyHash` digests + kid/verify-list labels) — asserted by scanning the canonical string + digest for every raw key value.
  - **Malformed stored Redis HMAC fails safe (should-fix, Codex):** a stored `codeHmac` that is non-hex or the wrong length must cause `otpHmacVerify` to return a clean verification FAILURE (treated as a wrong code) — it must **NOT** let `crypto.timingSafeEqual` throw on unequal buffer lengths and must **NOT** surface a 500. Length/hex-shape is validated before `timingSafeEqual` (mirrors the existing length-check-first pattern at the live verify sites). This does **not** alter the established attempt-limit counting or the dev/test `000000` bypass behaviour. Pinned by tests injecting a non-hex and a wrong-length stored value.
...
| R2-#1 | **Explicit, provider-owned OTP previous key (owner decision, post-R1).** Adds optional env `OTP_HMAC_KEY_PREVIOUS` + KeyProvider method `getOtpVerifyKids(): readonly string[]` returning exactly `[ACTIVE]` or `[ACTIVE, PREVIOUS]`. Signing uses ACTIVE only; verification iterates ONLY the provider-returned ordered list and **never** infers PREVIOUS from every non-active ring kid. Boot fails closed unless PREVIOUS is `otp-*` + present in `OTP_HMAC_KEYS` + `!= ACTIVE` + not in `DECOMMISSIONED_KIDS`; legacy-bridge mode requires PREVIOUS unset. Routine rotation retains exactly one PREVIOUS until **AT LEAST `T_lastsign + 24h + Δ`** (a MINIMUM overlap — the full longest challenge TTL after the last possible sign under the outgoing kid, plus a bounded deploy/clock-skew margin; **never** "≤24h" — Codex blocking #1), then removes it promptly; the compromised-key incident path keeps PREVIOUS unset (leaked `otp-legacy` is never a fallback; challenges invalidated immediately). Resolves the audit-flagged R1-API↔spec gap (the merged design said "verify ACTIVE then PREVIOUS" with no PREVIOUS designation). Dev/test bypass unchanged + separately guarded. | §3.3 (`getOtpVerifyKids`), §3.4 (env + boot validation), §3.6 (sign/verify + routine/incident overlap-removal rule), §3.12/§12 (incident), §11 (tests); plan §R2 |
| R2-#2 | **R2 parity fingerprint includes the ordered OTP verify list + a distinct capability (Codex blocking #2).** The R1 canonical fingerprint (`codeCapability` + ACTIVE pin/otp kids + hashed keyset) does NOT capture the PREVIOUS designation, so two services with identical ring+ACTIVE but different/unset `OTP_HMAC_KEY_PREVIOUS` would publish an identical fingerprint yet accept different OTPs; and an R1 image vs an R2 image with the same keys would appear compatible. R2 amends §3.9: the canonical formula adds `otpVerify: getOtpVerifyKids()` (the ORDERED `[ACTIVE]`/`[ACTIVE, PREVIOUS]` list, not the unordered ring) and bumps `codeCapability` to a DISTINCT `'v2-reader-otp-verify-v2'`. Web/worker parity then fails on a PREVIOUS-designation divergence (value mismatch) and R1-vs-R2 fails (capability mismatch); the fingerprint still contains no raw key bytes / HMAC material. | §3.9 (formula + parity gate), §3.3, §11 (tests); plan §R2 file list + tasks + type-consistency |

---

## 20. Pre-R1 staging recovery architecture (provider-grounded amendment)

> Design-level amendment after the staging P1001 diagnosis. The Web service's pre-deploy `prisma migrate deploy` failed with **Prisma P1001** (a database **connection** failure). Cause not isolated: at diagnosis time (June 2026) the Neon `staging` branch was archived/suspended + over the then-Free monthly compute allowance (plausible, not log-confirmed; Neon moved to the LAUNCH plan, usage-based + owner-set $20 limit with UNVERIFIED hard-stop enforcement, on 2026-07-01), and the Railway runtime `DATABASE_URL` targets the **pooled (`-pooler`)** host while migrations require the **direct** endpoint (a confirmed runbook mismatch — but pooled-host is *not* proven to be the connection-level P1001 cause). Operator detail: runbook §13; release ordering: plan "Pre-R1 staging recovery".

**20.1 Split the single P1 gate into P1a (runtime-recovery) + P1b (migration-readiness).** The existing runbook used one P1 gate that conflated *serving the pre-R1 baseline* (needs the **pooled runtime** endpoint + reachability) with *applying a migration* (needs the **direct** endpoint + migration role + grants). These are different endpoints and different evidence, so the design splits them: **P1a** verifies only what is needed to serve; **P1b** verifies the direct-migration readiness and must hold before any schema mutation. Both are satisfied by a **minimum read-only preflight** (reachability / host+database identity / injected role / required permissions via read-only catalog/grant inspection) — *no mutating operation* before the gate. The **first real migration is the execution confirmation, not the proof required to pass P1b.** **Endpoint rule:** there is **no unconditional "stop if pooled"** — `P1a` **requires** the verified **pooled** runtime endpoint, while `P1b` **requires** the verified **direct, non-`-pooler`** migration endpoint (pooled is prohibited only for the migration endpoint). **P9 ordering:** the P9 acceptance fixture is established **after** the pre-R1 baseline is serving (P8) and **before** any R1 migration or R1 deployment; R1 activation cannot begin while P9 is blocked. This split is a **reviewed runbook amendment** that must merge before execution.

**20.2 Runtime-pooled vs migration-direct — `MIGRATION_DATABASE_URL` (recommended long-term).** Today both the Prisma CLI (`prisma.config.ts` → `datasource.url = process.env["DATABASE_URL"]`) and the app runtime (`PrismaPg({ connectionString: process.env.DATABASE_URL })` in `src/worker.ts` + `src/api/plugins/prisma.ts`) read the **same** `DATABASE_URL`; there is no `directUrl`. The recommended design adds a **`MIGRATION_DATABASE_URL`** consumed **only** by the migration/CLI path (`prisma.config.ts`) — **mandatory in controlled staging/production migration contexts; optional only for local/test** — while **runtime keeps the pooled `DATABASE_URL`** — preserving pooled runtime connectivity AND direct migration access. **Fail-closed validation (migration context):** in **controlled staging/production migration contexts**, `MIGRATION_DATABASE_URL` **MUST be present** and **MUST validate as a direct, non-`-pooler`** URL — **missing or invalid fails closed** (the migration does not run). **There is NO fallback to pooled `DATABASE_URL` in staging/production**; a `MIGRATION_DATABASE_URL ?? DATABASE_URL` fallback is **explicit local/test-only behavior** (tested there), never the staging/production migration path — and in staging/production **unsetting the variable disables/fails the migration path** while runtime pooled connectivity is unaffected. `prisma.config.ts` governs the **Prisma CLI / migration** configuration, so this is a **migration-context** fail-closed — **not** an ordinary application boot failure (runtime app boot is unaffected, still on pooled `DATABASE_URL`). This is a **bounded** architecture/code/provider change requiring: spec/plan/runbook amendment; code/config + integration tests (migrate uses the migration URL; runtime unaffected); secret handling (the direct URL is a new injected secret, never printed); provider rollout; **rollback (honest): a provider rollback may remove/disable the migration path without affecting runtime pooled connectivity; a code rollback requires deliberately restoring the prior migration policy — never silently return staging/production migrations to pooled `DATABASE_URL`**; and **separate owner approval.** **Permanently removing the automatic pre-deploy migration is a separate deployment-policy decision — not part of incident recovery;** the temporary recovery disable of auto-migrate is reversible and recovery-scoped only. **Rejected:** repointing the runtime `DATABASE_URL` to the direct host (breaks legitimate runtime pooling).

**20.3 P8 rollback baseline — image rollback ≠ source rebuild.** Commit SHA + `package-lock.json` pin source + dependencies but do **not** guarantee a byte-identical build (Railpack/Nixpacks, Node/base image, build env may drift). The design therefore distinguishes **(a)** a retained Railway deployment/image with a Redeploy/Rollback action (byte-identical, **retention-limited** — can age out to `REMOVED`) from **(b)** a rebuild from a **protected GitHub branch fixed at the verified pre-R1 SHA `53bafac4716e8819b3a77ffb5a129bd6b25d59ef`** (source + deps identical, **durable**, not byte-identical). Railway deploys from a **configured branch, not a tag**, so the durable mechanism is the protected recovery branch (the branch is **not "literally immutable"** — GitHub provides no true immutability; enforce a **GitHub ruleset**: no force-push, no direct updates, with **exact-SHA verification before every deployment or rebuild**). **P8 INITIALLY requires BOTH** a serving baseline with full evidence (`/health` liveness + a bounded read-only `GET /api/v1/customer/categories` smoke validating status/shape/count only — never owner-data content) **AND** a **currently-visible Railway Redeploy/Rollback action** **AND** the durable protected-branch rebuild path — **the provider action is not merely a bonus.** **If the provider action disappears before R1 activation, P8 becomes BLOCKED and must be re-established.**

**20.4 Worker boundary.** Source proves persistent 60-second/hourly worker database activity (`RECONCILE_EVERY_MS=60000`, claim-stale hourly, `PROMOTE_PENDING_HOURS_EVERY_MS=60000`). These repeatable sweeps are the **leading identified persistent-activity mechanism, NOT a telemetry-proven sole cause** of the Neon compute usage. **Permanent worker remediation gates the worker restart only** — the bounded Web/R1 recovery proceeds with the worker Offline. **Plan state (owner-confirmed): Neon LAUNCH (usage-based) since 2026-07-01 + the owner-set $20 limit (enforcement UNVERIFIED); no further plan or spending-limit change is authorized.** (Consistent with Amendment #13 — the offline worker contributes parity only via `--verify-keyring-and-exit`.)

**Amendment ledger (round 5 — recovery architecture):**

| # | Correction | Where |
|---|---|---|
| R5-#1 | Split the single P1 gate into **P1a (runtime/pooled, serve only)** + **P1b (direct/migration, before any schema mutation)**; minimum read-only preflight permitted; first migration = execution confirmation, not P1b proof; **no unconditional "stop if pooled" — pooled REQUIRED for P1a, direct REQUIRED for P1b**; **P9 fixture established after the baseline serves and before any R1 migration/deploy** | §20.1; runbook §13.1–§13.3, §13.8 |
| R5-#2 | **`MIGRATION_DATABASE_URL`** recommended (migrate-direct, runtime-pooled); **MANDATORY in controlled staging/production migration contexts (optional only for local/test) — MUST be present + direct/non-`-pooler` or fail closed; NO staging/production fallback to pooled `DATABASE_URL`** (fallback is local/test-only); unsetting disables/fails the migration path (runtime unaffected); **honest rollback (provider removes/disables path; code rollback deliberately restores the prior policy; never silently return staging/prod migrations to pooled)**; `prisma.config.ts` = Prisma CLI/migration config (not app boot); permanent auto-migrate removal is a separate policy decision | §20.2; runbook §13.6; plan "Pre-R1 staging recovery" |
| R5-#3 | **P8 = protected recovery branch** (GitHub ruleset, not "literally immutable", SHA-verified before every deploy/rebuild) at the verified pre-R1 SHA + serving evidence (`/health` + read-only categories smoke) + **BOTH** the durable rebuild path **AND** a currently-visible Railway redeploy/rollback action (not a bonus); **P8 BLOCKED + re-established if the provider action disappears**; Railway source-branch/pre-deploy-command edits only STAGE and are applied exclusively by a Deploy click (apply-equals-deploy, verified 2026-07-03) — stage exactly the approved set with the deliberate deployment; a staged set containing anything unexpected is discarded | §20.3; runbook §13.4–§13.5 |
| R5-#5 | **Staleness + decisions:** P3 re-stated (all post-R1 `main` commits docs-only through the recorded tip; re-verify `src/` equivalence); D-R1 SUPERSEDED 2026-07-01 (Launch, usage-based, + $20 limit; enforcement UNVERIFIED; A1 = fresh owner usage/headroom check; no further plan/limit change); D-R6 LOCKED (P8 both-required) | runbook P3, §13.9 |
| R5-#4 | **Worker** sweeps = leading identified persistent-activity mechanism (not telemetry-proven sole cause); gates worker restart only; plan = Launch since 2026-07-01 (no further plan/limit change) | §20.4; runbook §11/§13.7 |
