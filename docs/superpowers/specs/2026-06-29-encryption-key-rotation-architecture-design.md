...
| R5-#5 | **Staleness + decisions:** P3 re-stated (all post-R1 `main` commits docs-only through the recorded tip; re-verify `src/` equivalence); D-R1 DECIDED (wait for Free reset; reconsider Launch only if it doesn't occur); D-R6 LOCKED (P8 both-required) | runbook P3, §13.9 |
| R5-#4 | **Worker** sweeps = leading identified persistent-activity mechanism (not telemetry-proven sole cause); gates worker restart only; no paid upgrade (wait for Free reset) | §20.4; runbook §11/§13.7 |
