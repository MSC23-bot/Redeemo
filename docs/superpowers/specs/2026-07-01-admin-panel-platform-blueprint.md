...
- **Maker-checker / four-eyes** on a single mutation: absent. The shipped `AdminApproval` submit/approve split is **workflow-role separation, not dual-control** (one admin can claim and approve); do not mislabel it as four-eyes anywhere in this blueprint.
...
| Maker-checker countersign by a DIFFERENT admin [NET-NEW; existing approval split is role-separation, not four-eyes] | Single-actor irreversible mass mutate [OUT] |
...
| M22 | **Operational Status** (Platform) | Bounded in-app status (BC-1). | PARTIAL (signals exist; read surface net-new) | BC-1 | v1 read panels vs gated Incident model. |
| M23 | **Feature flags / config** (Platform) | Read/observe config and gate state. | FUTURE (minimal); config is env today | BC-1 | Toggle-in-UI is a later decision. |
...
- **Data-export safety:** a blueprint-wide export contract (PII redaction, `isTestData`/QA/`DELETED` exclusion via the shared eligibility helper, `CommunicationLog.payload` never selected, branch PIN never exposed). CSV export exists (M3 redemptions) but no cross-cutting contract.
...
- **Segregation of duties as policy (not per-module):** the shipped approval submit/approve split is role-separation, not four-eyes; maker-checker is net-new; money-sensitive actions follow SALES-initiates / FINANCE-processes.
...
## 16. Open decision register (PROPOSED; not approved)

Approved process decisions: D1, D2, D14. All below are PROPOSED and separately gated.
...
**Freshness caveat:** merged is the floor, not the bar; every admin module is merged-not-staging-accepted; deployed SHA, admin-web deploy state, staging admin-OTP delivery, `STORAGE_ENABLED`/`EMAIL_ENABLED` state, and the DPIA gate state are not readable read-only and must be re-verified before reliance.
