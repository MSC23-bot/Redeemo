...
| M5 | **Merchant 360** (Relationships) | One merchant's deep WORKSPACE (Section 6.1), distinct from the Directory LIST (M3). Tabs: Overview, Business identity, Branches, Vouchers, Performance, Documents, Staff & roles, Comms, Commercial, Audit, Insights. Edit-on-behalf per the correction model (D37) + the D38 reclassification. Customer-facing banners (merchant cover + per-branch banner / logo) are SHOWN on Business identity + Branches (D40). Nav-entry lands on a picker when no merchant is selected (never a dead link). | PARTIAL (rich read substrate; many admin actions net-new) | BC-5, BC-2, BC-4 | Cross-check table Section 6.3. List-vs-workspace + banners + nav-entry per D40 (4.2 / 6.2). |
...
- **Tabs:** Overview, Business identity, Branches, Vouchers, Performance, Documents, Staff & roles, Comms, Commercial, Audit, Insights (mapping to the panes above and the Section 6.3 authority cross-check).
- **Edit-on-behalf:** per the correction model (D37, see 6.1.4) and the capability reclassification (D38, see 3.2 / 6.3). COSMETIC corrections apply directly; MATERIAL corrections (voucher value / saving, business / trading name) route to the merchant.
...
- **Enriched table:** logo + business name + trading name; category + subcategory; town + region; branch count; lifecycle status + verification status; ACTIVITY block = active-voucher count + redemption count + rating + a DORMANT flag (the supply-to-demand link: a merchant with live vouchers but no recent redemptions is dormant supply); commercial block = featured / campaign status + contract status + renewal / expiry; per-row actions.
...
- **Richer filters** across lifecycle, verification, region, category, activity / dormancy, commercial status.
- **Export CSV** (real).
