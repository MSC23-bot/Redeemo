...
- **Security hardening outcomes (design the outcome, not an approved mechanism):** idle-session timeout, step-up re-auth before destructive / lifecycle actions, and an IP allow-list are proposed launch-hardening outcomes (cross-ref D5); show the gate, do not lock a specific session / auth mechanism (D4).
