...
0. **Capture the access token BEFORE anything else.** `const token = getAccessToken()` at the very top of `signOut`, BEFORE step 1's `resetSessionState` nulls it. This captured value is the authoritative revoke proof (step 2); capturing it into a local const decouples the revoke from the client-side token-null.
1. **Invalidate + clear client state.** `await resetSessionState(qc)` - epoch bump (4.3 step 1), token null + `refreshInFlight` reset (step 2), `cancelQueries` (step 3), `clear` (step 4). In-memory and cached state are gone immediately, independent of any network call. A stale in-flight request cannot repopulate anything (epoch guard).
2. **Invoke the BFF logout with the captured token under a STRICT bounded timeout.** `authApi.logout(token)` (the step-0 captured value) -> `POST /api/merchant-auth/logout`, wrapped in a bounded timeout (`AbortController` + timer). The BFF ALWAYS attempts `clearSessionCookie` regardless of the revoke result. The SIGNED access-token JWT is the authoritative, tamper-proof revoke identity (Codex Wave 12 round-3 correction, backend design 3.3): the backend VERIFIES the JWT signature (expiry IGNORED - a validly-signed even-expired token proves the caller held THIS session; the explicit expired-token logout policy), extracts `sessionId`/`entityId` from the SIGNED claims (never the unsigned cookie), and DELs the session key UNCONDITIONALLY - which WINS against a concurrent refresh rotation (a refresh-token-possession-only revoke could LOSE that race, Opus F1). The cookie's refresh secret is forwarded too as a fallback for the rare no-token case. This (a) removes the token-null ordering dependency by capturing the token first (step 0), (b) is proof-bearing via the signature so a tampered/forged cookie cannot revoke another merchant or allow an unauthenticated body-only logout, and (c) has no expired-token gap (expiry ignored for logout). `authApi.logout` MUST forward the captured token.
