...
2. **Invoke the BFF logout with a STRICT bounded timeout.** `POST /api/merchant-auth/logout` (via `authApi.logout`), wrapped in a bounded timeout (a few seconds, e.g. an `AbortController` + timer). The BFF ALWAYS attempts `clearSessionCookie` regardless of the backend revoke result (cookie clearance is unconditional on the BFF response path). CRITICAL ORDERING NOTE (adversarial review F1): clearing the client token in step 1 (`setAccessToken(null)`) must NOT be allowed to disable the backend revoke. Today `authApi.logout(getAccessToken())` forwards the access token as a Bearer, and the BFF logout only calls the backend revoke `if (authorization)` (`logout/route.ts:15`) - so calling `authApi.logout()` after the token is already null would silently skip the revoke entirely (a regression from today, and it would kill the backend-durability design's whole revoke trigger). The RESOLUTION (see backend design 3.3): the BFF logout derives session identity from the httpOnly COOKIE (`readSessionCookie()` -> `sessionId`/`entityId`, mirroring the `/refresh` route's existing body-based pattern) and revokes body-based, NOT from the forwarded Bearer. This removes the token-ordering dependency (the cookie is still present until the response clears it, so clearing the in-memory token first is safe) AND closes a latent gap where an already-expired access token at idle-logout time would fail `authenticateMerchant` and silently no-op the revoke. `authApi.logout` therefore does not need to forward the access token for revoke purposes; the cookie is the durable identity.
