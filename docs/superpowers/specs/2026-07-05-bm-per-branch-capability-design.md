...
- Block ABSENT (older backend during deploy skew, or loading): `branchCapability` reads `undefined` -> the BM arm of `effectiveCanManage` is false -> BM-eligible controls stay hidden, exactly today's behaviour. Skew can only under-show, never over-show. The OWNER arm is profile-role-driven and independent of the branch payload, so owners retain full controls against any older/absent/malformed block - owner experience is skew-immune BY FORMULA, not by assumption (pinned by the §12 matrix).
...
- OLD backend + NEW frontend: no `viewerCapabilities` block on branch payloads -> the BM arm of `effectiveCanManage` is false -> assigned BMs keep today's (restricted) experience; OWNERS unaffected BY FORMULA (the OWNER arm never consults the branch block - §5/§6). NO functional regression for any role; strictly no over-grant. No compatibility bridge is required (unlike #377's pin field, where absence LOST existing owner-visible state; here absence merely delays a new grant).
