...
0. **Terminal cooperative stop signal.** `stop()` sets a **terminal** `stopped` flag BEFORE the drain (never reset). The tick loop checks `stopped` **before every sweep and after each awaited sweep** (breaking immediately); Phase B checks `isStopping()` **before starting each row AND before every separately-awaited DB/Redis/alert op within a row** (an atomic single-tx row is one awaited op, not broken mid-tx — since the owner-approved 2026-07-03 Option C amendment this covers BOTH pending-hours' `promoteOnePendingHours` AND the stale-claim row, whose `lastStaleAlertAt` CAS + `adminNotify` bell now commit or roll back together in one bounded transaction); and the failure path's `alertDegraded`/recovery launch sites are guarded by **`!isStopping()` immediately adjacent to the seam** (a failed sweep still logs synchronously and updates backoff, but is never reclassified SUCCESS and launches no alert). So once stop is requested NO new sweep, Phase-B row, in-row op, or alert starts. This does **NOT** cancel an already-running Prisma/Redis operation; it only prevents NEW work (a force-close below makes an in-flight op reject).
...
5. **Equally-safe process-termination fallback.** If `forceJoin` also times out, the real `shutdown()`'s `finally { process.exit(exitCode) }` (`worker.ts:114`) terminates the process: `stopped` being terminal + checked before every sweep, after each awaited sweep, before every Phase-B row, before every separately-awaited in-row DB/Redis/alert op, and immediately adjacent to every degraded/recovery alert launch guarantees NO new DB/Redis/alert op can start regardless (already-running operations are not cancelled — the force-close makes them reject), and `process.exit` destroys every remaining handle. A late rejection is caught per-row / by `forceJoin`, or by the process-level `unhandledRejection` LOG backstop (`worker.ts:128`) — never an unhandled crash.
...
This is a **mandatory pre-release gate** with **mutation-resistant** tests: stop during sweep 1 of several → later sweeps never start; stop during Phase B → no later row starts; drain-timeout → force-close → the current op settles/rejects safely; no DB/Redis op begins after closure; concurrent `stop()` share one result; **alert launch is terminal** (a failing sweep while stopping logs + backs off but launches no `alertDegraded`); **stop before a stale-claim row → its transaction never starts; a started stale-claim transaction commits or rolls back as ONE atomic op (Option C, 2026-07-03 — the former between-ops split and its benign duplicate-bell edge are retired: the CAS + bell are atomic and a CAS loss is a safe no-bell skip)**; and no timer re-arm / unhandled rejection / lingering handle (Redis-unavailable/half-open included). The source-grounded cooperative-stop cross-check table is in the implementation plan (Task A6).
...
- **Shutdown mid-tick (Codex):** the terminal cooperative stop signal is checked before every sweep, after each awaited sweep, before every Phase-B row, AND before every separately-awaited op within a row (both pending-hours and, per the 2026-07-03 Option C amendment, stale-claim run their row as ONE atomic bounded transaction), so no new sweep/row/op starts once stop is requested; alert launches are likewise guarded (no degraded/recovery alert after stop); a drain-timeout force-close makes the in-flight op REJECT (not cancelled), a bounded `forceJoin` awaits the unwinding tick catching the rejection, and `process.exit()` (`worker.ts:114`) is the equally-safe fallback (§4.6). No DB/Redis/alert op begins after resources close.
