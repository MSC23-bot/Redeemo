...
Graceful shutdown must terminate cleanly even if Redis is unavailable/half-open (a `commandTimeout` bounds the AWAIT but does NOT prove the socket/command is gone). Contract:
1. **Stop scheduler timers** (`scheduler.stop()`) and **prevent new producer/alert work** (`AlertSink.stop()`; no new enqueues).
2. **Bounded graceful close:** attempt `Queue.close()` + producer connection `quit()` within a bound.
3. **Force-close on expiry:** if the bound elapses, explicitly `disconnect()`/destroy the producer connection so the underlying socket + any stuck command are actually terminated — a **real force-close** (socket destroyed), NOT a cosmetic `Promise.race` that leaves work running. This also covers **shutdown while the producer is still in initial connect/readiness** (Redis down at cold start): the lazy connection + finite `retryStrategy` + force-`disconnect()` bound it, so shutdown cannot hang in readiness.
4. **No lingering handles:** after force-close no producer socket, timer, or pending event-loop handle remains, so the process can exit.
5. **AlertSink:** bounded drain of in-flight writes; after the bound, proceed to the controlled Prisma `$disconnect()`; all resulting rejections are caught. Shutdown never waits indefinitely for an alert or an enqueue. `recoveryPending` stays best-effort across restart.

This is a **mandatory pre-release gate** with a verified no-lingering-handles test (including Redis-unavailable/half-open shutdown).
...
| Graceful shutdown | `worker.ts:99-119` | preserved; scheduler stop added |

---

## 14. Fail-safe configuration (Codex #9)

For a running (staging/production) worker the maintenance config MUST be explicit — there is **no silent default into either an unbenchmarked adaptive cadence or the legacy 60s burn path.** Required env: `MAINTENANCE_FLOOR_IDLE_MS` (validated `> 300_000`), `MAINTENANCE_FLOOR_ACTIVE_MS`, `MAINTENANCE_PHASE_B_MAX_ITEMS`/`_BUDGET_MS`, per-sweep enable flags + `STATEMENT_TIMEOUT_MS < TX_TIMEOUT_MS`. **Fail-closed (Codex #7):** if required config is missing or invalid the validator **throws and the worker exits non-zero** (fail-startup, supervisor-visible), UNLESS an explicit owner-approved **`MAINTENANCE_MODE=disabled`** is set — in which case the worker boots with maintenance intentionally disabled + a loud structured log. There is **no silent half-running-with-maintenance-off default**, **no defaulted cadence**, and **no `MAINTENANCE_FLOOR_ENABLED=false → legacy 60s` path** (see §15 rollback and Codex #4). Local/test may pass explicit test config.
