...
1. **`await scheduler.stop()` FIRST — async, idempotent, bounded active-tick drain.** `stop()` sets `stopped` (no new tick), clears the armed timer, prevents any re-arm, and **tracks the ACTIVE TICK Promise (not just `tickInFlight`)**, awaiting it within a bounded drain window (`stopDrainMs`). If the tick settles → teardown proceeds normally; if the bound expires → `stop()` resolves a **TIMEOUT** result (`{ drained:false }`) reported to this shutdown sequence, which then performs the real Queue/Redis/Prisma force-close below — **the timeout does NOT cancel the tick** (a still-running tick is terminated by the resource force-close, not by the drain). Every resulting rejection is caught; **concurrent `stop()` calls share one drain Promise.** Worker shutdown AWAITS `scheduler.stop()` **before** closing Queue/Redis/Prisma. Then **prevent new producer/alert work** (`AlertSink.stop()`; no new enqueues).
...
| Graceful shutdown | `worker.ts:99-119` | preserved; **awaited bounded active-tick drain** (`await scheduler.stop()`) added before resource close |
