...
- *Guidance:* rationale: drives longer mid-week bookings · fallback: T2.
...
| **Category-specific risks** | **Health & Medical + Vets:** healthcare-advertising compliance (GDC/GMC/GOC) — no BOGO/% on clinical treatment; **owner needs a compliance sanity-check.** **Auto & Garage:** no BOGO. **Shopping / Home & Local Services:** the existing generic `20% Off` placeholders contradict the brand — **replace.** **Travel & Hotels:** higher floors; off-peak is a **Redeemo-approved exception** (admin-approved / TIME_LIMITED), NOT a default RMV restriction — use advance-booking + listed/capped blackout dates. |
| **Recommended MVP templates** | Seed **T1 (RECOMMENDED) for all 9** + replace the 4 generic placeholders; add T2/T3 where they help. Seed the schema-now fields (type/title/description/`minimumSaving`). **Terms:** the MVP target is the design-spec **§20 clause system** — `allowedFields` becomes `['selectedClauses','expiryDate']` and the per-template `default clauses:` lines seed the **pre-selected clauses** (NOT free-text `terms`). The richer `RmvTemplate` columns (`guidanceTip`/`imageGuidance`/`isRecommended`/`defaultEstimatedSaving` + `isActive`/`version`) land in the offer-engine migration (§13 / design-spec §22.3). |
