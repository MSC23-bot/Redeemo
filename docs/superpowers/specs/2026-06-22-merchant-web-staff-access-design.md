...
| D4 | Branch-scope enforcement | **Central enforcement via the two-resolver pattern (§4.1).** Keep `resolveAdminMerchant` **owner-only/unchanged** (it denies non-owners by construction); add a new `resolveMerchantContext` returning role + allowed-branch set + `canManageVouchers`; **migrate only the explicitly classified non-owner routes (§4.3) to `resolveMerchantContext` + a guard** - do NOT widen `resolveAdminMerchant`. **Invariant: never display a branch boundary the server does not enforce.** Do not surface the "Specific branches" option until enforcement covers every branch-specific route a non-owner can reach. |
...
| Central resolver | `resolveAdminMerchant(prisma, adminId)` (`merchant/shared.ts:72-90`) calls `getOwnerMembership` (OWNER only), returns `{ adminId, merchantId }`, throws on suspended. **Resolves OWNER only - a non-owner membership would not resolve at all today.** | **NEEDS-BACKEND (PR-A): leave `resolveAdminMerchant` owner-only/unchanged (safe deny); add a NEW `resolveMerchantContext` resolving any ACTIVE membership -> role + allowed branches + `canManageVouchers` (§4.1). Migrate only the §4.3-classified non-owner routes to it; do NOT widen `resolveAdminMerchant`.** |
...
- **PR-A - authz foundation + schema (no UI, no route migrated, non-owner login NOT live).** New `resolveMerchantContext` (role + allowed branches + `canManageVouchers`) **alongside the unchanged owner-only `resolveAdminMerchant`** (§4.1); the `canManageVouchers` migration; reusable guards (`assertCanManageVouchers` / `assertBranchAllowed` / `assertOwner` / role-elevation; `assertNotLastOwner` reuse). **Login/session - explicit boundary:** PR-A may add **helper- and test-only plumbing** for resolving a non-owner membership (the function that the login path will eventually call), **but must NOT allow a real non-owner to authenticate into a usable merchant session** - the live login/OTP/refresh path stays owner-only in PR-A (a non-owner cannot obtain a working session). No feature flag is introduced; the capability simply is not wired into the live auth path until PR-B. Unit tests pin the resolver, the guards, the non-owner-membership helper, and that no existing route (and no live login) changes behaviour. SHA-bound merge gate.
