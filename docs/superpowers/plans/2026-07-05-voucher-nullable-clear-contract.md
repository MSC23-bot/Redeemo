...
- Backward compatible in ONE direction only: the widened Zod schema is a strict superset, so the OLD frontend (which never sends null) against the NEW backend is unchanged behaviour. The opposite direction is not compatible - see the deploy-order bullet below.
