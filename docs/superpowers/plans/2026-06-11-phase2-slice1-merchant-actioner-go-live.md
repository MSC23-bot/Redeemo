...
> **SPLIT into M6a + M6b (owner-approved 2026-06-11, D-γ).** M6 is the highest-blast-radius milestone (auth + redemption verify + the one non-additive migration), so the additive safety work ships separately from the `MerchantAdmin.merchantId` drop.
>
> **✅ M6a SHIPPED (2026-06-11) — additive safety, NO schema change.** Admin `POST /admin/merchants/:id/{suspend,reactivate}` (cap `merchant:suspend`) — `$transaction` flips `status→SUSPENDED`/`onboardingStep→SUSPENDED` + transactional `MERCHANT_SUSPENDED` audit (actor/reason/before-after); after commit (best-effort): revoke all OWNER `authMerchant` + branch `authBranch` Redis sessions + refresh tokens (reuses the branch-user-deactivate precedent) and the **D-α cycle-refund sweep**. **SEC-M1:** `verifyRedemption` now reads **live** `voucher.merchant.status===ACTIVE` + `branch.isActive` (the service had NO status guard — only the routes' *cached* session check; the redeem path was already live). **SEC-M2 (full block, D-β):** `resolveAdminMerchant` + `refreshMerchantToken` refuse a SUSPENDED merchant — both read live status via a **`merchant.status` join added to `getOwnerMembership`** (membership source of truth, NOT the `MerchantAdmin.merchantId` column → no D-1 coupling; lenient `merchant?.status==='SUSPENDED'` so mocks/ACTIVE pass with no ripple). **D-α cycle-refund (as built):** suspend-time sweep resets `UserVoucherCycleState.isRedeemedInCurrentCycle` for in-flight (`isValidated=false`) **NORMAL-type** redemptions that are still the latest for their (user, voucher) — `TIME_LIMITED` + `REUSABLE` **EXCLUDED** (they bypass `UserVoucherCycleState`; their suspend-time handling is a follow-up). **Deferred from M6a:** the `MerchantAdmin.merchantId` drop + the login/OTP/completeLogin/branch-user/routes reroute (M6b); self-service-deactivate session revocation (D-δ); TIME_LIMITED/REUSABLE cycle-refund; true read-only access (full-block now). 7 src + 6 test files; 19 new M6a tests + 8 ripple-fixed mock files; admin/merchant/redemption/auth/shared/lib sweep 804/804; tsc 0-new. **M6b (the D-1 drop) — see the as-built note below.**
>
> **✅ M6b SHIPPED (2026-06-12) — the D-1 contract step, ONE internally-sequenced PR.** **Step 1 (reroute, additive — column still present):** `getOwnerMembership` joins `merchant.{status,businessName}`; `loginMerchant`/`verifyMerchantOtp`/`completeMerchantLogin` resolve the merchant via a new `resolveMerchantInfo` (OWNER membership) instead of `include:{merchant}`; the self-service deactivate/reactivate routes resolve via `getOwnerMembership` (behaviour-neutral, D-δ — no session revocation); `assertBranchOwnership` rerouted to **`resolveAdminMerchant`** (owner decision — extends the M6a SEC-M2 suspended-merchant full-block to branch-user management, since branch-user mgmt IS merchant management). **Step 2:** grep/static proof of **zero** remaining `admin.merchantId` / `admin.merchant` / `include:{merchant}`-on-merchantAdmin reads. **Step 3 (non-additive drop, migration `20260612014222_drop_merchant_admin_merchantid`):** a pre-flight guard refuses the drop if any `MerchantAdmin` lacks an ACTIVE OWNER membership (0 orphans in dev — guard passed), then drops the FK + the `@unique` index + the `merchantId` column; the schema also drops the `MerchantAdmin.merchant` relation + the reverse `Merchant.admin`; writes removed from `createMerchantDraft` + `seed.ts` + `seed-demo.ts`; ~10 real-DB test creates/cleanups rerouted to capture admin ids via the membership before deletion; the M1 "AUTH NON-REGRESSION — column KEPT" pin **inverted** to assert the column + relation are gone and the OWNER membership is the sole link. **`MerchantMembership` is now the SOLE ownership source of truth (D-1 end-state reached).** Affected + shared sweep (auth/merchant/admin/shared/redemption) **70 files / 613 tests green**; tsc 0-new (the 4 pre-existing `savings.service.test` baseline errors unchanged). **Down-path** (manual — Prisma is forward-only): re-add `merchantId TEXT` → backfill from each admin's ACTIVE OWNER membership → `SET NOT NULL` → recreate the unique index + FK (documented in the migration file header).
