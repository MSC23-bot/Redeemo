...
**Status:** APPROVED + PR-1 IN FLIGHT (2026-07-09). All DECISIONS in the addendum
(`docs/superpowers/specs/2026-07-09-loc-slice-3-pin-drop-addendum.md` §8) are adjudicated;
outcomes are recorded in that addendum's §9. PR-1 (backend) is implemented on branch
`feat/branch-location-trust-slice-3-backend`; PR-2 (merchant-web UI) remains open. Where a
task depended on a decision, the decision is now resolved (see the as-shipped note below).

> **As-shipped note (PR-1, 2026-07-09).** PR-1 ships: the `MERCHANT_CONFIRMED` enum +
> migration (dev-applied; joins the owner-gated §LOC-MIGRATE production queue), the
> `CONFIRMED_LOCATION_SET` widening + ranking/homeRail union sweep, the pure
> `pinWithinPostcodeArea` radius helper, the `dropBranchPin` service + `POST .../:id/pin-drop`
> route (OWNER-only; tightened from the addendum §1.1 recommendation) + the per-user
> `branchPinDrop` rate-limit tier, the admin serializer `merchant_pin_drop` source (null
> placeId tolerated), admin-web provenance ("Merchant-set pin") + zod widening, and the
> backend-proxied static-map endpoint `GET .../:id/map-preview` (DARK by default; usage-capped;
> key server-side only). It does NOT touch `apps/customer-app` (Task 7's customer-app half
> ships separately). **Merge ordering (hard): this PR must merge strictly AFTER the
> customer-app enum-tolerance fix is in the field (addendum §3.3 / §9.2 D-L6).**
