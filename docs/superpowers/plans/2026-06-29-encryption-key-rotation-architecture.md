...
1. **A1 — compute/cost headroom (updated 2026-07-03):** Neon is on the **LAUNCH plan (usage-based) since 2026-07-01** with the owner-set **$20 spending limit** (hard-stop enforcement UNVERIFIED — a budget signal, never guaranteed containment). A1 = a **fresh owner check of current Launch usage, spending headroom and project state immediately before P1a**; **no further plan or spending-limit change is authorized**.
...
3. **P1a — runtime-recovery preflight** (spec §20.1 / runbook §13.1): minimum **read-only** verify of the **pooled runtime** endpoint + database + reachability — enough to serve the pre-R1 baseline. No migration/mutation. — **Status: steps 1-3 (A1/A2/P1a) EXECUTED and PASSED 2026-07-03** (runbook §13.3.1 execution record); the steps below are NOT started.
4. **Short-term recovery** (auto-deploy disabled · worker Offline): protected recovery branch (GitHub ruleset; SHA verified before every deploy/rebuild) at the **verified pre-R1 SHA `53bafac4716e8819b3a77ffb5a129bd6b25d59ef`** — **✅ FULLY EXECUTED 2026-07-03 (runbook §13.5.1): branch + ruleset (D-R4a), then the Railway repoint + pre-deploy-hook removal staged together and applied by ONE owner-clicked Deploy (D-R4b + D-R5/D-E6); deployment `6d26b0b4…` successful; baseline SERVING; P8 ESTABLISHED. P9/P1b/R1 (steps 5+) NOT started** → **STAGE together: the Railway source-branch repoint AND the pre-deploy-migrate removal** (apply-equals-deploy, verified 2026-07-03 — Railway config edits only stage; the sole apply action is Deploy; verify the staged set is exactly these two approved changes, else Discard + STOP) → **one deliberate Deploy** (applies both + starts the recovery deployment; Railway runs no migration) → record **P8 evidence** (`/health` + read-only `GET /api/v1/customer/categories` smoke + deployment ID/SHA/timestamp + **BOTH** a currently-visible Railway redeploy/rollback action AND the durable protected-branch rebuild path — D-R6; runbook §13.5). ⇒ **P8 established.**
...
7. **Apply the R1 migration `20260629000000_keyring_fingerprint` on the DIRECT endpoint FIRST** (operator-run, R1 §3 / OD8) — only **after** P8 + **P9** + P1b; **THEN stage the Railway source-branch restoration to `main`/R1** (pre-deploy migrate stays absent) and apply it via the single deliberate R1 deployment's Deploy click (apply-equals-deploy — staging the restore earlier would deploy the R1 image against an unmigrated schema; runbook §13.8 steps 7-8).
...
- **`MIGRATION_DATABASE_URL` — mandatory for controlled staging/production migration; optional only for local/test.** The migration/CLI path (`prisma.config.ts`) uses it; runtime `PrismaPg` keeps the pooled `DATABASE_URL` (`src/worker.ts`, `src/api/plugins/prisma.ts` unchanged). A `MIGRATION_DATABASE_URL ?? DATABASE_URL` fallback is **local/test-only** — never the staging/production migration path.
...
- Tasks (future): config/unit test (migrate prefers the migration URL; runtime unaffected); loopback integration test; secret handling (new injected direct URL, never printed); Railway provider rollout; **rollback (honest): provider rollback may remove/disable the migration path without affecting runtime pooled connectivity; a code rollback deliberately restores the prior migration policy; never silently return staging/production migrations to pooled `DATABASE_URL`**; spec/plan/runbook amendment. **Separate owner approval required.** Permanent removal of the auto pre-deploy migrate is a **separate deployment-policy decision**, not bundled here.
...
**Stop-and-report (recovery):** no *mutating* DB op before the relevant gate (P1a serve / P1b migrate); read-only preflight only; no FURTHER plan or spending-limit change (Launch + $20 limit set 2026-07-01, enforcement UNVERIFIED); worker stays Offline until the permanent reduction + approval; R1 migration not applied before P8 + **P9** + P1b; Railway branch-change + auto-migrate-removal are separately-approved changes that only STAGE — they are applied exclusively by the deliberate deployment's single Deploy click (apply-equals-deploy, verified 2026-07-03), with auto-deploy disabled; do not rely on a retention-limited image rollback as the sole P8 basis; do not repoint runtime `DATABASE_URL`→direct; if P1001 persists after headroom + the direct path, STOP and re-diagnose.
