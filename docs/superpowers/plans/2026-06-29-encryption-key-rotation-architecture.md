...
- [ ] **Write failing tests:**
  ...
  - **verify overlap** — a code signed under the outgoing kid verifies WHILE `OTP_HMAC_KEY_PREVIOUS` is set and is REJECTED once it is unset (routine-window expiry / incident immediate-invalidation); **incident** — with PREVIOUS unset + `otp-legacy` denylisted, a leaked-key signature fails (no fallback).
...
- [ ] **Run → fail.**
- [ ] **Implement** `getOtpVerifyKids()` + `OTP_HMAC_KEY_PREVIOUS` boot validation in `keyring.ts` (fail-closed per §3.4); `otpHmacSign(challenge, code)` (ACTIVE only) + `otpHmacVerify(challenge, code, storedHmac)` (iterate `getOtpVerifyKids()` in order, `timingSafeEqual`, first-match); swap the 4 SIGN sites to `otpHmacSign` and the 3 VERIFY sites to `otpHmacVerify` (dev/test bypass branches preserved verbatim); extend `env.ts` for the OTP ring incl. `OTP_HMAC_KEY_PREVIOUS`. (Routine cutover sets `OTP_HMAC_KEY_PREVIOUS` to one outgoing kid for ≤24h then unsets it; the staging incident path leaves it UNSET — Operation A.)
- [ ] **Run → pass; allowlist guard passes; `tsc` clean.**
- [ ] **Commit** (`feat(auth): separate OTP-HMAC key ring from ENCRYPTION_KEY`).

**Merge gate:** allowlist guard proves all 7 sites swapped (4 sign / 3 verify); `getOtpVerifyKids()` ordered-list + PREVIOUS boot-validation pins green; both services parity-verified (published fingerprint); CI green; R1 already merged + live. **R2 release gating (hard): do NOT merge/deploy R2 until — R1 migration applied to Neon + R1 image deliberately deployed + Web/worker `KeyringFingerprint` parity verified + R1 staging acceptance complete.**
...
- **Type/name consistency:** `KeyProvider`/`getOtpHmacKey`/`getOtpVerifyKids`, `otpHmacSign`/`otpHmacVerify`, `encryptEnvelope`/`decryptEnvelope`/`parseEnvelope`, `ENCRYPTION_V2_WRITES_ENABLED`, `OTP_HMAC_KEY_ACTIVE`/`OTP_HMAC_KEY_PREVIOUS`, `KeyMigrationCheckpoint`/`KeyringFingerprint`/`KeyDecryptTelemetry`/`KeyringTelemetryHeartbeat`, `DECOMMISSIONED_KIDS`, the `v2:<kid>:<iv>:<tag>:<ct>` shape, and `pin-*`/`otp-*` match the spec verbatim.
