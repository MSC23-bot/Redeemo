...
7. **Apply the R1 migration `20260629000000` on the DIRECT endpoint FIRST** (operator-run, R1 §3 / OD8) — only **after** P8 + **P9** + P1b; **THEN stage the Railway source-branch restoration to `main`/R1** (pre-deploy migrate stays absent) and apply it via the single deliberate R1 deployment's Deploy click (apply-equals-deploy — staging the restore earlier would deploy the R1 image against an unmigrated schema; runbook §13.8 steps 7-8).
