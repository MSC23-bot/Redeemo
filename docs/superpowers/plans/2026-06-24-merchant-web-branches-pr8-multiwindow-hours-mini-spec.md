...
- VALIDATOR (`openingHours.ts`) changes: replace the duplicate-day reject with per-day grouping; ADD within-day NO-OVERLAP validation across the day's N windows (after sorting by openTime; treat an overnight `close<open` window's span as `[open, 24:00) + [00:00, close)` of the next day for overlap purposes within the day's own rows); keep the per-row format / `24:00`-close / zero-length / closed-day rules; forbid `isClosed:true` mixed with open windows. OVERLAP BOUNDARY (locked): overlap is detected on HALF-OPEN intervals `[open, close)` to match `isOpenNow`; abutting windows where `prev.close === next.open` (e.g. `09:00-14:00` then `14:00-23:00`) are ACCEPTED (no gap, no double-open). Do NOT use a closed-interval check (it would wrongly reject legitimate back-to-back windows).
...
Backend:
- Validator: multi-window per day accepted; within-day overlap rejected; closed-day = one `isClosed` row (no times) accepted, `isClosed` mixed with open windows rejected; `24:00`-close accepted; `open === close` rejected; ordering normalised.
