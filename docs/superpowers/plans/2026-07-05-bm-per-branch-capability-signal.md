...
All flips consume the spec §5 EFFECTIVE capability, never the raw block: `effectiveCanManage = role === 'OWNER' || (role === 'BRANCH_MANAGER' && branchCapability?.canManage === true)` - owner controls are profile-role-driven and skew-immune by formula; a forged or erroneous `true` cannot widen STAFF or unknown roles (role-qualified); the branch block is parsed via the spec §5 non-throwing `.catch(undefined)` contract.
