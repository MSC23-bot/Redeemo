...
- Merchant `createBranch` route: compute `stageForApproval` by BRANCH COUNT, NOT merchant status. Only the merchant's FIRST branch (the main branch, `existingNonDeletedBranchCount === 0`) is created INSTANT (it is covered by the merchant onboarding approval, which reviews the merchant + the main branch). EVERY subsequent merchant-created branch (`existingNonDeletedBranchCount >= 1`) goes through the BRANCH_CREATE approval lane (PENDING_CREATE + `AdminApproval`), INCLUDING a second branch added pre-live during onboarding. Rationale: keying on `merchant.status === 'ACTIVE'` was too broad: a pre-live merchant who already has their first onboarding branch could add a second branch that would be created INSTANT (unreviewed as a branch) and then go live the moment the merchant is approved. The count-based rule closes that hole: the only instant create is the first/main branch; every additional branch gets its own per-branch admin review. Auth stays `resolveAdminMerchant` (OWNER-only). STOP-AND-REPORT if live code blocks a PENDING_CREATE branch from coexisting with a pre-live (onboarding) merchant: the onboarding go-live gate (`approveMerchant`) checks only the MAIN branch's `isBranchLocationConfirmed`, and the customer-exclusion keeps a pending branch invisible, so a pending second branch is NOT expected to block onboarding: report if any onboarding invariant says otherwise.
...
- Auto-main safety: a PENDING_CREATE branch must NEVER be auto-promoted to main. Under the count-based discriminator the ONLY instant create is `existingNonDeletedBranchCount === 0` (the first/main branch); every staged (PENDING_CREATE) branch is by definition `existingNonDeletedBranchCount >= 1`, so the existing `isMainBranch = existingCount === 0` can NEVER make a pending branch main. PR-5 additionally makes the `existingCount`/main logic count only non-pending, non-deleted branches, and the customer cold-open nearest-branch / `isMainBranch` defaults only ever resolve to LIVE branches. Recorded invariant: main is only ever a LIVE branch, and only the first branch is ever instant.
...
Backend:
- CREATE of a SUBSEQUENT branch (`existingNonDeletedBranchCount >= 1`, whether the merchant is pre-live OR live) -> branch `lifecycleStatus = PENDING_CREATE`, `isActive = false`, an `AdminApproval(BRANCH_CREATE)` created, NOT auto-main, customer-INVISIBLE on BOTH the feeds AND the picker AND the resolver AND favourites AND review/redemption.
- CREATE of the FIRST branch (`existingNonDeletedBranchCount === 0`, the main branch during onboarding) -> INSTANT LIVE (no pending, no approval), auto-main as today.
- CREATE of a SECOND branch PRE-LIVE during onboarding (the Codex case) -> PENDING_CREATE + `AdminApproval(BRANCH_CREATE)` (NOT instant); it stays customer-invisible and does NOT go live when the merchant onboarding approval lands (it needs its own BRANCH_CREATE approval).
...
- DECISION RECORDED (not a stop): ApprovalType naming resolved to TWO values `BRANCH_CREATE` + `BRANCH_CLOSE` (section 3b). The pending-vs-instant create discriminator is BRANCH COUNT (section 4a): only the merchant's FIRST branch (`existingNonDeletedBranchCount === 0`, the main branch) is instant; EVERY subsequent merchant-created branch (`>= 1`, including a second branch added pre-live during onboarding) stages for BRANCH_CREATE approval; admin-on-behalf is always instant. The status enum is `PENDING_CREATE / LIVE / PENDING_CLOSE / CLOSED` (default LIVE). Pending-create is `isActive=false` + status-excluded (defence-in-depth). Flip any of these at review if the owner prefers.
...
|---|---|---|---|
| 1 | `createBranchCore` INSTANT-LIVE: auto-main on `existingCount===0`, postcode->`POSTCODE_CENTROID`, no `AdminApproval`. `createBranch` is OWNER-only (`resolveAdminMerchant`). | Parameterised by BRANCH COUNT: a subsequent merchant-created branch (`existingNonDeletedBranchCount >= 1`, pre-live OR live) stages `lifecycleStatus=PENDING_CREATE` + `isActive=false` + `AdminApproval(BRANCH_CREATE)`; only the first/main branch (`existingNonDeletedBranchCount === 0`) + admin-on-behalf stay INSTANT LIVE. Auth unchanged (OWNER-only). | Only the first/main branch is instant; auto-main never promotes a pending branch. |
...
| 7 | `createBranchCore` is shared by the merchant route AND admin create-draft-on-behalf. | The `stageForApproval` param keeps the admin path instant; only a merchant self-created SUBSEQUENT branch (`existingNonDeletedBranchCount >= 1`) stages. | Prevents the admin from approving their own draft; only the first/main branch is instant on the merchant path. |
