...
`POST /api/v1/merchant/branches/:id/location/search`, body `{ query: string }`, registered on the merchant branches plugin (the PR-3 stack). Gated EXACTLY like `/photos/upload`: `resolveMerchantContext(app.prisma, req.user.sub)` then `assertCanManageBranch(ctx, id)` (OWNER any / assigned BRANCH_MANAGER / STAFF denied) BEFORE calling `searchPlaces`. Calls `searchPlaces(query, { source: 'merchant_portal' })` so usage lands in the reserved bucket. Maps `GooglePlaceCandidate[]` to a CLIENT-SAFE DTO that STRIPS `latitude`/`longitude`/`googleMapsUrl`: e.g. `{ candidateToken, name, formattedAddress }` (a server-issued opaque token, NOT raw coords). The candidate -> coordinate binding is held server-side as a CACHE ONLY (the candidate's coords from the original search are stashed short-lived in Redis keyed by merchant + token, and read back at pick time) so lat/lng are never echoed to the client. The pick-time read MUST be a cache lookup, NOT a fresh billable Google call (no Place Details / second `searchPlaces`): a fresh call would double usage and pull in a separate billable endpoint (a blueprint §11.4.2 stop-and-approve). If the cache entry has expired, the merchant re-searches; do not silently re-call Google. `placeId` MAY be returned (it is an opaque Google id, not location-revealing) or kept server-side; default keep it server-side behind the token for tidiness.
...
- LIVE branch: through the EXISTING reviewed edit lane `createBranchEditRequest` (address is SENSITIVE; never bypass identity review). The submit additionally carries the server-side Google metadata (the chosen candidate token / placeId + the Google lat/lng) as ADMIN-REVIEW METADATA stashed in `BranchPendingEdit.proposedChanges` (a metadata sub-key) + an `AuditLog.metadata` entry - NOT as a confirmed Branch confidence. The address fields apply through the existing lane as normal; the postcode resolver stamps `POSTCODE_CENTROID` (CORRECT - non-confirmed).
- Onboarding (draft-window, direct write): the autofilled address saves directly as today; `locationConfidence` stays `POSTCODE_CENTROID` (non-confirmed). The Google lat/lng/placeId are recorded as admin-review metadata via an `AuditLog` entry on the branch (alongside the existing `BRANCH_CREATED`/`BRANCH_UPDATED` audit; the direct create/edit does not produce a `BranchPendingEdit`). NO `ADDRESS_GEOCODED`. (If the onboarding direct-write path turns out to have no convenient audit sink for this metadata, that is a small additive decision to resolve at implementation, not a schema change.)
...
|---|---|---|---|
| Location search (the new route) | Allowed (any branch) | Allowed (assigned branch) | Denied |
...
The search route is `assertCanManageBranch`-gated (same as the SENSITIVE address edit it feeds). Server-enforced. The key + lat/lng are server-only (never sent to the client).
...
Backend:
- The search route returns candidates for a query (mocked `searchPlaces`), gated `assertCanManageBranch` (OWNER/assigned-BM 200; unassigned-BM + STAFF 403); the wire DTO NEVER contains `latitude`/`longitude`/`googleMapsUrl` (the never-expose invariant).
...
|---|---|---|---|
| 1 | `searchPlaces(query,{source})` funded; only caller is the admin CLI (`admin_cli`); `merchant_portal` bucket reserved + test-pinned; key server-only. | New server-proxied merchant route `POST /branches/:id/location/search` calling `searchPlaces(query,{source:'merchant_portal'})`, `assertCanManageBranch`-gated, returning a client-safe DTO (no lat/lng). | First merchant-side Google caller; honours the brainstorm-first lock (this mini-spec). |
