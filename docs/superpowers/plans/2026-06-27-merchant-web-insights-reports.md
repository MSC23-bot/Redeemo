...
- `/busy-times` → **privacy-mode-aware** (the SERVER owns the mode; the client never derives suppression from raw counts):
  - **intensity mode** (DEFAULT, and the only mode unless PR-0a D6's recorded policy approves exact counts): `{ mode:'intensity', grid:[{day(Mon=0..Sun=6), daypart(0-5), intensity}], busiest:{day,daypart}|null }`. `intensity` is a **small server-computed ordinal band** (relative volume, e.g. `0=none..3=high`); **no `logged`/raw count and no raw peak value appear in the payload**; `busiest` is the peak **cell location only** (day+daypart, **never a count**) and is `null` whenever the peak cell could be near-empty (conservative omission; the threshold is **deferred to PR-0a D6**).
  - **exact mode** (ONLY under an approved PR-0a D6 policy): `{ mode:'exact', grid:[{day, daypart, logged, intensity}], busiest:{day,daypart}|null }`.
  - **unavailable:** `{ available:false }` when a safe intensity output cannot be produced.
  NOTE: `day` is **Mon=0..Sun=6** (prototype visual row order) - DELIBERATELY DIFFERENT from `getLondonClock`'s `0=Sun..6=Sat`; do **not** reuse that helper's output without remapping (pinned in Task A4). The privacy-mode-aware payload + the no-`logged`/no-raw-peak invariant are pinned by route/payload negative tests in Task A7.
...
- [ ] **D6 - Busy-times sparse-cell + peak policy** (gates the release-1 busy-times exact-count behaviour, spec §1.7): whether exact sparse counts may be shown; the minimum cohort/cell threshold; intensity-only vs hidden treatment; the **"Busiest" badge threshold**; anti-inference across filters. Until D6 exists, busy-times uses the documented **safe fallback** (server-side intensity-only, no exact sparse counts; raw counts/peak never reach the browser) or is unavailable. The payload is **privacy-mode-aware** (`mode:'intensity'` by default; exact counts only under an approved D6 policy; else `{available:false}`, §2.7). The service-level fallback is built in Task A4; the route/payload **negative** tests (no `logged`/exact/raw-peak under narrowed filters) live in Task A7.
...
- [ ] **Busy-times pin (real DB):** the default payload is **`mode:'intensity'`** with **no `logged`/exact-count field present**; assert the grid `day` index is `Mon=0..Sun=6` by seeding ONE redemption per target cell (a known London Monday → the `day===0` cell becomes non-zero intensity; Sunday → `day===6`); each seeded row contributes to exactly one of the six dayparts (no gap/overlap - every other cell stays at the zero band); after-midnight → its actual London calendar day. Assert via the emitted **intensity** grid, never via raw counts. Do NOT reuse `getLondonClock`'s `0=Sun..6=Sat` without remapping.
...
- [ ] **Busy-times sparse-cell (safe fallback until PR-0a D6, real DB):** suppression is **server-side**; the default payload is **`mode:'intensity'`** - ordinal bands for every cell (**never exact counts**), **no `logged`/raw count and no raw peak value** - until D6's recorded policy approves exact counts (`mode:'exact'`); when a safe intensity output cannot be produced the service returns **`{available:false}`**. The "Busiest" badge is the server-provided peak **location** (`busiest:{day,daypart}`, never a count), **omitted (`null`) whenever its peak cell could be near-empty** (conservative omission; threshold **deferred to PR-0a D6**; not decided here, spec §1.7). The service builds this fallback here; the route/payload **negative** tests (no `logged`/exact/raw-peak field, including under narrowed date/branch/voucher filters) live in **Task A7**.
...
- [ ] **Busy-times privacy-mode negative test (critical, real DB):** the default `/busy-times` payload is `mode:'intensity'` (or `{available:false}`) and contains **no `logged`, exact-count, or raw-peak field anywhere in the grid or `busiest`** - asserted on the full grid AND repeated with date, branch, and voucher-type filters **narrowed** (a merchant cannot induce exact counts or reconstruct a suppressed cell via any filter combination); `mode:'exact'` is returned **only** when the recorded PR-0a D6 exact-count policy is set (default off → never `exact`); `busiest` is location-only (never a count) and `null` for a near-empty peak. The client is sent no raw counts to threshold on.
...
- [ ] Failing test: schemas parse the section 2.7 responses incl. Decimal via `z.coerce.number()`, the **per-KPI `comparison` objects** (each nullable), the `{available:false}` gated repeat-rate variant, and the **privacy-mode-aware `/busy-times` union** - a `mode`-discriminated union of `{mode:'intensity', grid:[{day,daypart,intensity}], busiest:{day,daypart}|null}` (the intensity-grid item is **`.strict()`** so an unexpected `logged`/exact-count field **fails to parse**) and `{mode:'exact', grid:[{day,daypart,logged,intensity}], busiest:{day,daypart}|null}`, plus `{available:false}`; the client renders bands from `mode:'intensity'` and never thresholds on raw counts. Implement `apiFetch`-based `getInsightsOverview/Trend/Vouchers/Branches/BusyTimes/Validation/Customers` + `insightsExportUrl()`/`insightsReportUrl()`. Commit.
...
3. **Busy-times sparse-cell:** added PR-0a D6 (exact-vs-intensity-vs-hidden + thresholds + anti-inference); until then a server-side safe fallback (intensity-only, no exact sparse counts, raw counts/peak never reach the browser); route/payload negative tests in A7 (see §16.3 for the privacy-mode-aware payload follow-up). §3 D6/A4/A7 + spec §1.7.
...
- **Traceability (should-fix):** dropped the then-inaccurate "/A7" busy-times citation (the anti-bypass test lived only in A4 at that point). **Superseded in §16.3** - A7 now carries the route/payload negative tests, so the A4/A7 citation is restored.
...
### 16.3 Fourth review (Codex) - busy-times privacy-mode contract + dedup
One blocking contract inconsistency remained after §16.2: plan §2.7 still defined `/busy-times` as `{grid:[{day,daypart,logged}], busiest}`, exposing the exact per-cell count and contradicting the intensity-only fallback in §1.7/A4. Patched the contract to be explicitly **privacy-mode-aware** (closed scope; no behaviour beyond the contract):
- **Mode-tagged payload (§2.7):** `mode:'intensity'` by default (ordinal bands, no `logged`/raw count, no raw peak; `busiest` is location-only and `null` for a near-empty peak); `mode:'exact'` only under an approved PR-0a D6 policy; `{available:false}` when a safe intensity output cannot be produced. The server owns the mode; the client never derives suppression from raw counts.
- **Aggregation + fallback (A4):** the default payload is `mode:'intensity'`; the busy-times pin asserts the intensity grid (no `logged` field) and bucketing; the service builds the fallback, route/payload tests move to A7.
- **Route/payload negative tests (A7):** the fallback response carries no `logged`/exact-count/raw-peak field, on the full grid AND under narrowed date/branch/voucher filters; `mode:'exact'` only when the recorded D6 policy is set.
- **Schemas (B1):** a `mode`-discriminated union; the intensity-grid item is `.strict()` so an unexpected `logged` fails to parse; plus `{available:false}`.
- **Rendering (B6):** renders from server intensity bands; exact counts only in `mode:'exact'`; explicit unavailable state; the badge comes from the server `busiest` location; the client never thresholds.
- **Spec §15.2** busy-times line + traceability (§2.7/§3 D6/A4/A7/B1/B6, spec §1.7/§15.2) updated; the A4/A7 citation restored (A7 now carries the negative tests).
- **Dedup:** removed the duplicated PR-C-non-executability closing paragraph (it appeared twice across §16/§16.2); one canonical closing remains below.
