...
- **Summary cards (prototype 01/12):** Locations = `branches.length`; Open right now = count where `openNow(branch.openingHours)` (Europe/London); With Redeemo = count where `isWithRedeemo(branch)` = `branch.isActive === true && branch.locationConfidence === 'MANUALLY_CONFIRMED'`, GATED by the merchant-lifecycle guard. CANONICAL lifecycle source (nominated, single source of truth): `useSession().approvalStatus` (already on the session, sourced from the merchant record; no extra fetch). Merchant is "Live" iff `approvalStatus === 'APPROVED'`. When `approvalStatus !== 'APPROVED'` (PENDING / CHANGES_REQUESTED / REJECTED / null = pre-Live), the merchant is not on Redeemo at all, so render With Redeemo = 0 (never the raw per-branch `MANUALLY_CONFIRMED` count). If a shared `deriveStatusPill`-style "live" helper is later extracted, reuse it for consistency with the shell pill; for PR-1 the `approvalStatus === 'APPROVED'` predicate is the contract.
...
- **Tests first:** renders summary cards from a mocked list (Locations/Open/With-Redeemo correct incl. merchant-pre-Live → With-Redeemo 0); renders rows with Setup PIN set/not-set WITHOUT exposing the pin value; row click navigates; empty + error states; "Add branch" is disabled and fires no network call.
...
- **Pill/role rendering (reconciled prototype + spec):** reuse the EXISTING `RoleAccessCell` treatment from `components/staff/StaffTable.tsx` (the established Staff & Access table), which renders BOTH the role AND the access distinction the umbrella spec requires: a portal member shows their colour-coded role chip (`RoleChip(ROLE_LABEL[role])`: OWNER=red, BRANCH_MANAGER=navy, STAFF=teal) PLUS a "Portal" `AccessPill` (the "Portal + app" identity), and an Invited/Pending marker when `claimed===false`; an app-only `BranchUser` shows an "App user" role chip PLUS an "App" `AccessPill` (the "App access" identity). This satisfies the prototype's role-coloured pills (screenshots 05/06/09) AND the spec/Codex requirement to distinguish portal vs app-only staff. Show name + status; NEVER expose passwordHash/PIN/phone. Email-correlation de-dup is DISPLAY-ONLY (do not merge identities). Extract `AccessPill`/`RoleChip` into `components/branches/StaffPills.tsx` (or reuse `RoleAccessCell` directly if it fits the panel layout); if extraction risks the Staff surface, duplicate the small styled spans and note it.
...
- **Tests first:** portal + app users merged + filtered by branchId; pills correct; `allBranches` member shows at every branch; non-owner (staff 403) → panel hidden + `isOwner=false`; no secret fields rendered.
...
| With-Redeemo count (01) | `isActive`+`locationConfidence` on list | F2 + `withRedeemo.ts` | Included | count + merchant-pre-Live guard | merchant lifecycle not in payload (use shell signal) |
...
- With-Redeemo lifecycle guard: nominated ONE canonical source: `useSession().approvalStatus`; Live iff `=== 'APPROVED'`; otherwise the count is 0.
