...
Context: a "Redeemo for Business" Merchant Portal prototype was previously created in Claude Design; it holds valuable context about Redeemo's brand, merchant workflows, terminology, modules, interactions and visual language. The supplied reference pack REPRESENTS that prototype; the new Admin project must not assume any cross-project memory of it. The Admin Panel should reference that context where Claude Design supports it. This document establishes what Claude Design actually supports (no assumptions) and recommends the safest mechanism.
...
2. the shared **design-system foundations** (`docs/design/merchant-portal/design-system/tokens.css` + the two font files + `.../upload-bundle/2026-06-10-brand-design-system-foundations-design.md`) as the exact brand source: **OWNER-LOCAL/untracked**, attach from the owner's checkout if present. The tokens + brand-spec are optional (the exact hexes + 60-30-10 + no-emoji/no-em-dash are already in master-context §B as text), but the two FONT FILES are REQUIRED for exact type fidelity: a font name is not the font, so if the Mustica Pro + Lato files are missing or Claude Design cannot ingest them, STOP and report, and any fallback typeface needs explicit owner approval.
