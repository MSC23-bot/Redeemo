...
**Primary mechanism:** a concise **Merchant-prototype reference pack** uploaded into the Admin session, consisting of:
1. the shared **design-system foundations** (`docs/design/merchant-portal/design-system/tokens.css` + the two fonts under `.../design-system/fonts/` + `docs/design/merchant-portal/upload-bundle/2026-06-10-brand-design-system-foundations-design.md`) as the brand source (identical across products);
2. the three **design-system reference pages** `docs/design/merchant-portal/design-system/{01-foundations.html, 02-components.html, 03-voucher-blocks.html}` as the PRIMARY visual anchors: they render the palette, base components, status-pill language and the voucher-card visual directly (better than a screenshot, because they are the real components);
3. a short **merchant-reference summary** (one or two pages): the merchant nav groups + module list, the lifecycle/approval status vocabulary, the voucher-card and status-pill language, and the terminology glossary, so the Admin session speaks the same language;
4. **a handful of real merchant screenshots as secondary visual anchors:** the git-tracked PNGs under `docs/superpowers/prototype-references/merchant-web-branches/` (branch overview, branch detail, edit/close modals) and `.../merchant-web-insights-reports/` (dashboard/reports), plus the handoff `screenshots/audit-home.png`. Honesty note: there is NO ready curated PNG set of the shell / LIVE dashboard / onboarding-checklist as such; those live inside the clickable `Redeemo for Business.dc.html` in the handoff zip. If one of those specific screens is wanted as an anchor, open the `.dc.html` and capture it; do not assume a pre-cropped PNG exists. Attach a handful (not the whole prototype).
