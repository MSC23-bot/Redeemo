...
- **Brand (identical across products):** page background white; cream `#FFF9F5` for identity/warm surfaces; navy `#010C35` for text/depth; a **Rose->coral gradient** (`#E20C04` -> `#E84A00`) used sparingly (<= ~10% of a screen, mostly the single primary action). Two fonts only: **Mustica Pro SemiBold** (display/headings) + **Lato** (body/labels). 60-30-10 balance; one dominant element and one primary action per view; WCAG AA; 44px targets; no emojis; no long dashes (use colons, semicolons, parentheses, hyphens); British English.
