...
**Safety, privacy and legal boundaries (design these visibly):** operator PII access to customers is gated and audited (reveal-on-demand); customer deletion is anonymise-in-place (some fields are retained by design); analytics that touch customer behavioural or demographic data are default-off and gated ("not available" until a privacy gate opens); a branch's redemption PIN is masked by default and is revealed on demand (reason-required and audited) or reset only on the Merchant 360 Branches tab (owner-decided D46, reversing the prior "PIN never shown" rule), the PIN value never appears in logs or audit, and it is never shown on any other surface; legal/agreement content is owner/legal-owned and external; documents open via short-lived signed links, never raw storage paths.
...
- **Branch:** three orthogonal axes: active/inactive (reversible) vs soft-deleted (not reversed by the active toggle) vs lifecycle (pending-create, live, pending-close, closed); plus location confidence (manually-confirmed, address-geocoded, postcode-centroid, needs-review). A branch redemption PIN is masked by default and is revealed on demand (reason-required and audited) or reset only on the Merchant 360 Branches tab (owner-decided D46); the PIN value never appears in logs or audit and it is never shown on any other surface.
...
- **Redemption:** issued (with an 8-char code shown to the customer) then validated in-store by staff. There is NO reversal/void state today (that is a gated, schema-dependent future). Customer identity on redemptions is role-gated (owner-decided D48, replacing the flat "never show identity" rule): AGGREGATE surfaces never show an individual customer identity for any role, but INDIVIDUAL redemption identity (a per-merchant Redemptions tab, a single-redemption detail, and a redemption-code lookup) is role-gated (visible-by-default for customer-facing roles, reveal-with-reason for restricted roles, no-access for others) and EVERY view or reveal is audited; savings are per-redemption, aggregates are anonymous.
...
- **Redemption codes:** synthetic 8-char like `A7K2P9X4`. A branch PIN is masked by default everywhere and is only revealed on demand (reason-required and audited) or reset on the Merchant 360 Branches tab (owner-decided D46): render it as a masked "PIN hidden - tap Reveal (reason required)" on the Branches tab, and simply "PIN managed on the Branches tab" on any other surface; use a synthetic PIN value if a reveal state is shown, and never put the PIN value in any audit / activity row.
...
- Customer PII gated + reveal-on-demand + audited; no customer-home map; branch redemption PIN masked by default, revealed on demand (reason + audit) or reset only on the Merchant 360 Branches tab, PIN value never in logs / audit and never shown elsewhere (D46); redemption customer identity role-gated and always audited (aggregate surfaces never show an individual identity; individual identity is visible-by-default / reveal-with-reason / no-access by role) (D48).
