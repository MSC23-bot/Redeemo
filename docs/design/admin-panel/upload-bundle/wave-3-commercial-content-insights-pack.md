...
- Commercial: create a campaign to set locations/cost to add merchants to (payment status gate) to schedule live; create a featured placement to see it blocked until payment status is confirmed; mint a promo code; grant a complimentary subscription with reason + audit.
