...
Reproducibility note: `docs/design/merchant-portal/**` currently has zero tracked files (this Admin bundle under `docs/design/admin-panel/` IS tracked); everything under `docs/design/merchant-portal/**` is OWNER-LOCAL. The `docs/superpowers/prototype-references/*.png` (22 files) ARE tracked, as is the merchant product blueprint under `docs/superpowers/specs/`. A clean-clone reviewer therefore has the Admin blueprint, this bundle, the 22 PNGs and the merchant product blueprint, but NOT the merchant brand/design-system files, the merchant prompt pack or the handoff.
...
| Merchant brand tokens | `docs/design/merchant-portal/design-system/tokens.css` | OWNER-LOCAL | exact colour/spacing/radii/shadow tokens | brand fidelity | CP-0 if present; NOT required (the exact hex values are in master §B as text) |
...
| Merchant fonts (2 families) | `docs/design/merchant-portal/design-system/fonts/{MusticaPro-SemiBold.otf, Lato-*.ttf}` | OWNER-LOCAL | the exact two typefaces | type fidelity | REQUIRED for exact type fidelity (a font name is not the font); if missing or un-ingestable, STOP and report; a fallback typeface needs explicit owner approval |
...
| Merchant product blueprint | `docs/superpowers/specs/2026-06-16-merchant-portal-product-blueprint.md` | TRACKED | full merchant product spec | already distilled into the summary above | LOOKUP ONLY; not uploaded |
| Merchant Claude Design prompt pack | `docs/design/merchant-portal/upload-bundle/2026-06-17-merchant-portal-claude-design-prompt-pack.md` | OWNER-LOCAL | the merchant prompt operationalisation | already distilled into the summary above | LOOKUP ONLY; not uploaded |
...
```
If the OWNER-LOCAL rows are missing (e.g. a clean clone), proceed with the tracked summary + master-context §B brand + the tracked PNGs; the owner-local tokens/HTML are enrichment, not blockers. The two font FILES are the exception: exact type fidelity requires the actual Mustica Pro + Lato files, so if they are missing (or Claude Design cannot ingest them), STOP and report before claiming exact type, and a fallback typeface needs explicit owner approval. **Any decision to bring the merchant design-system files (or the handoff) into the repo is a SEPARATE, explicit preservation/commit decision, not part of this D14 bundle; do not silently commit the large handoff, fonts, prototype HTML, or unrelated untracked merchant artifacts.**
