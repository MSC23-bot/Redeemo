...
4. **Postcode area label.** `PC2AddressScreen.tsx` postcode lookup displayed `admin_district` from postcodes.io — for HD1 4RU that returned "Kirklees" (the metropolitan borough council), which most users don't identify with. Now prefers `parliamentary_constituency` (returns "Huddersfield" for HD1), with `admin_district` then `admin_ward` as fallbacks. Display-only — only the raw postcode is sent to the backend, so no contract change.
