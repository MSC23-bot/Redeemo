...
4. **Postcode area label.** `PC2AddressScreen.tsx` postcode lookup displayed `admin_district` from postcodes.io — for HD1 4RU that returned "Kirklees" (the metropolitan borough council); a first-pass fix swapped to `parliamentary_constituency` ("Huddersfield" for HD1) but failed for rural postcodes (CO7 0UB → "Harwich and North Essex" — covers many villages). Final heuristic in `pickAreaLabel()` picks the most user-recognisable place name: prefers civil `parish` when it's a real name (rural villages — CO7 0UB → "Brightlingsea"); skips the postcodes.io placeholder format `"X, unparished area"` returned for unparished urban zones; falls back to `parliamentary_constituency` (urban — HD1 4RU → "Huddersfield"); then `admin_district` / `admin_ward`. Display-only — only the raw postcode is sent to the backend, so no contract change.
