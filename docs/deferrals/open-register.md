...
| §FB.1 | /for-businesses mobile active-section highlighting: the business navbar's scroll-spy indicator (island underline + quick-nav red state) has no equivalent inside the mobile hamburger/float menus; surface the active section there (Codex non-blocking note on PR #534, merge 457bef03) | OPEN, Tier 1 |
| §FB.2 | /for-businesses portal image sizing: pane captures ship at 1386px-wide intrinsic with `sizes="100vw"` hints flagged by next/image (oversized transfer on small screens); right-size `sizes`/variants for the portal panes and journey strips (Codex non-blocking note on PR #534, merge 457bef03) | OPEN, Tier 1 |
