...
- **§LOC-3** (Slice 3) - BACKEND MERGED 2026-07-10 (PR #452 `ea1cf979`, after the locked-order tolerance PR #451 `9d056f73`): pin-drop endpoint + `MERCHANT_CONFIRMED` tier + proxied static-map preview; design addendum APPROVED in-branch with all four owner decisions. REMAINING: the merchant-web pin-drop UI (draggable pin over the proxied static map, shaded 1km disc, wired to the endpoint; plan PR-2). GATED: merchant-web build slot (next in sequence).
