...
**§ADM-DEFER (OPENED 2026-07-10, owner direction):** Admin S4 (Members & Subscriptions read-only v1) implementation and S5 (Ops Home v1) are DEFERRED. The S4 plan-of-record is merged (`docs/superpowers/plans/2026-07-09-admin-members-subscriptions-readonly-v1.md`, #449 `d99305e2`, owner decisions folded in) and implementation-ready when the customer-facing phase resumes. Next strategic priority = merchant-recruitment funnel (discovery not yet started; owner to approve plan/sequencing first). Trigger to pick S4/S5 back up: owner call at the customer-launch runway.
