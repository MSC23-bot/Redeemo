...
import { MapPin, CreditCard, TrendingUp, XCircle } from '@/design-system/icons'
import { Text } from '@/design-system/Text'
import { spacing, radius, elevation } from '@/design-system/tokens'
import { FadeInDown } from '@/design-system/motion/FadeIn'

// §Savings Rebaseline spec §State 1 / §State 2 "Benefit cards".
// Four cards on State 1 (free user), three on State 2 (subscriber-empty —
// the "Cancel anytime" card is dropped because they've already
// committed). Stagger fade-down entrance keeps the surface feeling
// alive without overwhelming the hero CTA.
...
export function BenefitCards({ variant }: { variant: Variant }) {
  ...
  return (
    <View style={styles.container} testID={`savings-benefit-cards-${variant}`}>
      {cards.map((card, i) => {
        const Icon = card.icon
        return (
          <FadeInDown key={card.key} delay={200 + i * 80}>
            <View style={styles.card} testID={`savings-benefit-card-${card.key}`}>
