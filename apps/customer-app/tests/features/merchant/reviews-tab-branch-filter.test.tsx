...
jest.mock('@/features/merchant/hooks/useMerchantReviews', () => ({
  useReviewSummary:   () => ({ data: { averageRating: 4.5, totalReviews: 3, distribution: { 5: 2, 4: 1, 3: 0, 2: 0, 1: 0 } }, isLoading: false }),
...
describe('ReviewsTab branch-filter toggle', () => {
  beforeEach(() => mockUseMerchantReviews.mockClear())

  it('default state queries with branchId=current', () => {
    renderTab()
    expect(mockUseMerchantReviews).toHaveBeenLastCalledWith('m1', expect.objectContaining({ branchId: 'b1' }))
  })

  it('toggle to All branches removes the branchId filter', () => {
