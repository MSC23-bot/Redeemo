...
describe('resolveRedirect', () => {
  ...
  it('sends in-progress profile to correct step', () => {
    expect(resolveRedirect({ status: 'authed', onboarding: { profileCompletion: 'in_progress', furthestStep: 'pc2', phoneVerifiedAtLeastOnce: true }, user: { emailVerified: true, phoneVerified: true }, currentGroup: 'auth', currentSegment: 'welcome' })).toBe('/(auth)/profile-completion/pc2')
  })
})
