...
import { decodeAdminJwt } from '@/lib/auth/session'

// POST /api/admin-auth/refresh (no body) -> backend /refresh. Reads the
// httpOnly cookie, calls the backend with { refreshToken, sessionId, entityId },
// ROTATES the cookie with the new (single-use) refresh token, and returns ONLY
// { accessToken } to the browser. On any failure the cookie is cleared (the
// session is dead) and the backend status is forwarded so the client can
// hard-logout.
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const claims = decodeAdminJwt(d.accessToken)
