...
// surface (PR #28). Without this enrichment, the seeded merchants render
// with all the null-fallbacks (no logo, no banner, no photos, no opening
// hours, "Closed today", no rating pill, no website button, single branch
// → Branches tab hidden). Every UI component is correctly wired; this
// just gives them data to render.
//
...
async function seedDemoMerchantEnrichment(): Promise<void> {
  ...
  // Pattern: closed Monday; Tue-Fri lunch+dinner split (close 15:00, open
  // 17:00); Sat-Sun open all day. dayOfWeek 0 = Sunday … 6 = Saturday.
  ...
  // Unblocks BranchesTab QA (the tab is intentionally hidden on
  // single-branch merchants per PR #28 plan §1.1).
  await prisma.branch.upsert({
  ...
  // Opening hours for Colchester — slightly different schedule for variety.
  ...
  // 4 reviews on the main branch — varied ratings averaging ~4.5★.
  ...
  for (let i = 0; i < reviewerIds.length; i++) {
    ...
    await prisma.review.upsert({
      ...
      update: { rating: review.rating, comment: review.comment, isHidden: false, isDeleted: false },
...
async function main() {
  ...
  // Gives the QA + demo flow a "golden" merchant page exercising every
  // Merchant Profile UI component (logo / banner / website / opening hours
  // / photos / reviews / multi-branch). Scoped tightly to one merchant —
  // do not generalise.
  await seedDemoMerchantEnrichment()

  // ── Backfill denormalised merchant counts ──
  // Runs AFTER the demo enrichment so the new Colchester branch counts
  // toward Covelum's per-city presence.
