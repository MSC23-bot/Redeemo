...
/**
 * Bulletproof slug uniquifier: produces a stable, unique slug for every Locality.
 *
 * Strategy (escalation ladder — first non-colliding candidate wins):
 *   1. base                                         — slugify(name)
 *   2. base + '-' + slugify(ladDistrict)            — disambiguates same-name across LADs
 *   3. base + '-' + slugify(lad) + '-' + slugify(country)
 *                                                   — disambiguates cross-country same-name same-LAD
 *   4. base + '-' + slugify(lad) + '-' + slugify(country) + '-2/3/4/…'
 *                                                   — final fallback for unresolved ties
 *                                                     (e.g. "Holt" + "Holt (Dorset)" both in Dorset LAD,
 *                                                     both in England — neither LAD nor country splits them)
 *
 * `used` is the running Set of every slug emitted so far in this run. The escalation
 * ladder reads against it and the chosen candidate is added before return.
 *
 * Slugs are intended to be stable across regenerations as long as the source
 * (name, ladDistrict, country) tuples remain stable. Numeric-suffix ordering depends
 * on iteration order of the `groups` Map, which is JS-spec-ordered (insertion order =
 * ONSPD postcode-stream order), so quarterly regenerations should keep the same suffix
 * assignments for the same source rows.
 */
function uniqueSlug(name: string, ladDistrict: string, country: string, used: Set<string>): string {
  const base = slugify(name)
  if (!used.has(base)) { used.add(base); return base }

  const withLad = `${base}-${slugify(ladDistrict)}`
  if (!used.has(withLad)) { used.add(withLad); return withLad }

  const withCountry = `${withLad}-${slugify(country)}`
  if (!used.has(withCountry)) { used.add(withCountry); return withCountry }

  for (let i = 2; i < 100; i++) {
    const candidate = `${withCountry}-${i}`
    if (!used.has(candidate)) { used.add(candidate); return candidate }
  }
  throw new Error(`uniqueSlug: > 99 collisions for name="${name}" lad="${ladDistrict}" country="${country}" — investigate source data`)
}
...
async function main() {
  ...
  const slugEscalationCounts = { base: 0, withLad: 0, withCountry: 0, numeric: 0 }
  ...
  for (const { rows, bua, resolved } of groups.values()) {
    ...
    const sizeBefore = emittedSlugs.size
    const slug = uniqueSlug(name, ladDistrict, country, emittedSlugs)
    if (emittedSlugs.size !== sizeBefore + 1) {
      throw new Error(`uniqueSlug failed to add a new slug for name="${name}" lad="${ladDistrict}" country="${country}"`)
    }
    // Track which escalation rung this slug landed on (for the run summary).
    if (slug === slugify(name)) slugEscalationCounts.base++
    else if (slug === `${slugify(name)}-${slugify(ladDistrict)}`) slugEscalationCounts.withLad++
    else if (slug === `${slugify(name)}-${slugify(ladDistrict)}-${slugify(country)}`) slugEscalationCounts.withCountry++
    else slugEscalationCounts.numeric++
  ...
  console.log('')
  console.log('=== Slug uniquification (escalation ladder) ===')
  console.log(`  base only (no collision):                   ${slugEscalationCounts.base}`)
  console.log(`  base + LAD suffix:                          ${slugEscalationCounts.withLad}`)
  console.log(`  base + LAD + country suffix:                ${slugEscalationCounts.withCountry}`)
  console.log(`  base + LAD + country + numeric suffix:      ${slugEscalationCounts.numeric}`)
  console.log(`  Unique slugs emitted: ${emittedSlugs.size} (matches localities total: ${emittedSlugs.size === localities.length})`)
  console.log('')
