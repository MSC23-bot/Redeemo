...
describe('merchant auth routes', () => {
  ...
  beforeEach(async () => {
    ...
    app.decorate('prisma', {
      ...
      // Staff & Access PR-B B8 (cutover): resolveMerchantInfo now uses
      // getActiveMembership (findMany, ANY active role), so per-test mocks provide
      // findMany returning the full ActiveMembership shape.
  ...
  it('POST /api/v1/merchant/auth/login returns OTP_REQUIRED for first login', async () => {
    ...
    app.prisma.merchantMembership.findMany = vi.fn().mockResolvedValue([{
      id: 'mm1', merchantId: 'm1', merchantAdminId: 'ma1', role: 'OWNER', allBranches: true, canManageVouchers: false, merchant: { status: 'ACTIVE', businessName: 'Test Co' }, branches: [],
    }])
  ...
  it('POST /api/v1/merchant/auth/login returns 403 MERCHANT_DEACTIVATED for an INACTIVE merchant', async () => {
    ...
    app.prisma.merchantMembership.findMany = vi.fn().mockResolvedValue([{
      id: 'mm1', merchantId: 'm1', merchantAdminId: 'ma1', role: 'OWNER', allBranches: true, canManageVouchers: false, merchant: { status: 'INACTIVE', businessName: 'Test Co' }, branches: [],
    }])
    const res = await app.inject({
  ...
  it('POST /api/v1/merchant/auth/login returns 403 for suspended merchant', async () => {
    ...
    app.prisma.merchantMembership.findMany = vi.fn().mockResolvedValue([{
      id: 'mm1', merchantId: 'm1', merchantAdminId: 'ma1', role: 'OWNER', allBranches: true, canManageVouchers: false, merchant: { status: 'SUSPENDED', businessName: 'Test Co' }, branches: [],
    }])
