...
- **Screenshot model — platform asymmetry, locked framing.** Android = **prevention** (`FLAG_SECURE` via `preventScreenCaptureAsync` blocks capture system-wide; recents previews go black). iOS = **detect-and-react ONLY** (Apple does not expose any screenshot-prevention API; `addScreenshotListener` fires AFTER the screenshot has been written to Photos). On iOS the captured image WILL contain the unblurred QR + 8-char code — the blur paints AFTER the fact. The user-visible trust signal on iOS is the **live screen itself** (animated gradient border, pulsing LIVE dot, ticking en-GB London datetime, validated chip transition) — a static screenshot freezes all of these and trained staff can spot it. **Never describe iOS as "screenshot prevention"** in spec, plan, PR description, marketing, or in-app copy. The on-screen banner copy ("Screenshot taken. Staff verify only the live screen. Tap the QR to show again.") and the deferred `screenshot-flag` telemetry are the post-fact mitigations, not prevention.
