...
Test counts at branch tip after §AE wave: backend vitest 112/112 across the redemption + voucher-detail surface; customer-app jest **444/444** across all 26 voucher suites (PulsingDot 5, QRCodeBlock 9, useRedemptionPolling 5, useBrightnessBoost 5, useAutoHideTimer 7, useScreenshotGuard 13, useScreenCaptureProtection 6, ShowToStaff 24, voucher-detail-redeem-flow **61** (was 54; +5 §AE + +2 ShowToStaff propagation pre-existing), redemption-details-card **28** (was 21; +7 presentation-window state machine), presentationWindow utils **16** new, success-popup, voucher-detail-back-navigation, branch-picker-sheet-voucher, collapsed-header-scroll, cycle-rules-card, merchant-row, pin-entry-sheet, product-copy, use-redeem, use-redemption-lockout, use-time-limited, formatRedemptionCode, voucher api, redemption api). `tsc --noEmit` zero new errors in `src/api/` (single pre-existing unrelated `branchName: string | null` error in VoucherDetailScreen.tsx remains).
