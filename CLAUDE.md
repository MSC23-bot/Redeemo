...
7. **Merchant approval is gated.** Gates the approval service actually enforces before go-live (verified 2026-07-02 during the Admin Panel prototype): at least one branch exists; the contract is signed; two mandatory (RMV) vouchers are configured; a main branch is set; and the main branch location is confirmed. Intended but NOT yet enforced by code (aspirational, reconcile later): mandatory profile fields filled, verification documents uploaded, and a branch staff user assigned. (The prototype's go-live checklist shows the five enforced gates and flags the aspirational items as intended-not-enforced; see blueprint section 8.1 / D36.)
