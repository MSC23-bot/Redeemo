...
- **§AE Presentation-window gate (locked 2026-05-08, owner direction PR #49):** the redemption code + QR + Show-to-Staff entry point on Voucher Detail are LIVE for **2 hours** after `redeemedAt`. After the window closes — OR once staff has validated — Voucher Detail FULLY HIDES the code surface (no QR, no manual code, no "for your records" text) and instead surfaces a tilted brand-red "VOUCHER REDEEMED" seal with "Renews on <date>", a muted hero (opacity 0.55), and the tip line *"You'll be able to find this code in Profile → Redemption History."* The 2h window covers the realistic span between tapping Redeem and reaching the till; persisting beyond it lets a customer return hours later and re-show staff a code that's already been claimed. Backend cycle quota is the hard gate (already in place); the window is the social-engineering / re-show gate. Hook design is **setTimeout-at-expiry** (single timer fires once at the boundary), NOT polling — backgrounding-safe, JS-thread-light. Defense-in-depth: handler-side guard (`if (showRedeemedSeal) return` in `VoucherDetailScreen.onShowToStaff`) refuses to mount ShowToStaff even if the hidden CTA is somehow reached. Constant: `PRESENTATION_WINDOW_MS = 2 * 60 * 60 * 1000` in `apps/customer-app/src/features/voucher/utils/presentationWindow.ts`. The full polished SVG circular stamp + washed-out coupon visual treatment + merchant-profile redeemed-card + Profile → Redemption History full surface + backend `presentationExpiresAt` mirror are all DEFERRED to §Q1 / §AF — M3 ships the text-based seal + state-machine + opacity treatment as the working stop-gap. Pinned by 16 unit tests on the helper/hook (`presentationWindow.test.ts`) + state-machine tests on the card + screen-level §AE pins. §Q6 cycle-rollover invariant (load-bearing gate is `voucher.isRedeemedThisCycle`, NOT `lastRedemption` data presence) remains intact through the window flip.
...
- **§AE5 user-facing helper copy (locked 2026-05-08, refined 2026-05-09 from PR #49 device QA).** Two calm copy lines on the redeemed-state surface so the disappearing code never feels broken:
  - **In-window**, near the Show-to-Staff CTA: *"Available to show staff until \<D Mon, HH:mm\>."* (e.g. *"Available to show staff until 9 May, 00:55."*). en-GB / Europe/London / 24-hour. **Always includes the date** (locked 2026-05-09 device QA — "until 22:55" alone was confusing because the redeemed time was ALSO 22:55; date-inclusive disambiguates the cross-midnight case). Defensive fallback to *"You can show this code to staff for 2 hours after redeeming."* on malformed ISO. testID `redemption-details-availability-helper`. Hermes-robust formatter (`formatExpiryLine`) uses `Intl.DateTimeFormat.formatToParts` numeric extraction + a hardcoded English month-name array (root-cause fix for the device-QA report where `toLocaleTimeString({ timeZone: 'Europe/London' })` was silently rendering the redeemed time, not the +2h expiry — same Hermes/CLDR fragility class as the `weekday: 'short'` issue locked in `londonNow.ts`).
  - **Out-of-window AND not validated**, near the history tip: *"Staff handoff window ended. Your redemption details are saved below."* testID `redemption-details-window-ended`. Suppressed when validated — the validated pill carries the message; surfacing "ended" alongside reads as a contradiction.
  - The existing *"You'll be able to find this code in Profile → Redemption History."* tip remains as the routing pointer.
  - 12 copy pins in `redemption-details-card.test.tsx`: full date+time format / fallback on malformed / out-of-window calm explanation / coexistence with history tip / validated suppresses "available to show" / validated suppresses "ended-window" / in-window suppresses "ended-window" / day-boundary edge case (23:30 UTC → 02:30 next-day BST) / **PR #49 device-QA scenario (21:55 UTC → 9 May, 00:55 BST cross-midnight)** / day-numeric without leading zero / month transition (31 May → 1 Jun) / explicit Hermes-robust formatter assertions.
...
Test counts at branch tip after §AE5 + §AE6 + §AE6.2 + Hermes-robust helper-copy fix: backend vitest 112/112; customer-app jest **470/470** across all 26 voucher suites (presentationWindow utils 16, redemption-details-card **39** (+12 helper-copy pins; was 28), voucher-detail-redeem-flow **76** (+6 §AE6 + +9 §AE6.2 = +15 from baseline 61), success-popup, ShowToStaff, useScreenCaptureProtection, useScreenshotGuard, plus other voucher suites unchanged). `tsc --noEmit` zero new errors in `src/api/` (single pre-existing unrelated `branchName: string | null` error in VoucherDetailScreen.tsx remains).
