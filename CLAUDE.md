...
- **§AE5 user-facing helper copy (locked 2026-05-08, PR #49 review wave 3).** Two calm copy lines on the redeemed-state surface so the disappearing code never feels broken:
  - **In-window**, near the Show-to-Staff CTA: *"Available to show staff until \<HH:mm\>."* (en-GB / Europe/London 24-hour clock — exact expiry time computed from `redeemedAt + PRESENTATION_WINDOW_MS`). Defensive fallback to *"You can show this code to staff for 2 hours after redeeming."* on malformed ISO. testID `redemption-details-availability-helper`.
  ...
  - 8 copy pins in `redemption-details-card.test.tsx`: helper format / fallback on malformed / out-of-window calm explanation / coexistence with history tip / validated-suppression of "available to show" / validated-suppression of "ended-window" / in-window does-not-show ended-window / day-boundary clock formatting.
...
Test counts at branch tip after §AE5 + §AE6 + helper-copy waves: backend vitest 112/112; customer-app jest **458/458** across all 26 voucher suites (presentationWindow utils 16 new, redemption-details-card **36** (was 28; +8 helper-copy pins), voucher-detail-redeem-flow **67** (was 61; +6 §AE6 screen-capture pins), success-popup, ShowToStaff, useScreenCaptureProtection, useScreenshotGuard, plus other voucher suites unchanged). `tsc --noEmit` zero new errors in `src/api/` (single pre-existing unrelated `branchName: string | null` error in VoucherDetailScreen.tsx remains).
